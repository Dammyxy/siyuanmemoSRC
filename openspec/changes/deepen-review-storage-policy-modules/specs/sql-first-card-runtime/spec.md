...
The system SHALL NOT use binary snapshot or legacy unified-card storage as an active startup, recovery, Browser, Queue, NeuralRoam, Review mutation, truth import, or projection rebuild data source after the one-time migration cutover has been retired. Backend startup projection rebuild decisions SHALL be owned by the focused Storage Bootstrap Runtime while preserving SQL-first truth/projection authority. Storage policy declarations SHALL be catalog-owned while remaining export-compatible from the backend RPC contract surface.
...
#### Scenario: Storage bootstrap runtime owns startup projection rebuild decisions
- **WHEN** backend startup needs to determine whether a temp SQL projection can be used or must be rebuilt from durable truth
- **THEN** the system SHALL make that decision through the Storage Bootstrap Runtime and SHALL preserve the same fail-closed legacy snapshot restrictions

#### Scenario: Storage policy declarations stay export-compatible
- **WHEN** existing SQL-first runtime code imports MessagePack truth family or SQL projection policy declarations from `backend-rpc.ts`
- **THEN** those imports SHALL keep working after policy declarations move into the storage policy catalog module
