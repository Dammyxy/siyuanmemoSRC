...
- **AND** the contract SHALL expose a front/back contract with front before reveal and back after reveal
- **AND** the contract SHALL expose required receipt statuses for source block identity and quick-symbol evidence
...
- **AND** the contract SHALL expose a diagnostic for the stale route metadata
...
- **THEN** Review SHALL prepare the quick card front side from the contract

#### Scenario: Revealed answer prepares back
- **WHEN** Review displays a quick-symbol card after the answer is revealed
- **THEN** Review SHALL prepare the quick card back side from the contract
