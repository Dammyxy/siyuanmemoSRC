...
### Requirement: Plugin-owned AI Workbench UI
**Reason**: SiYuan core Agent now owns AI chat and reasoning, so a plugin-owned parallel AI Workbench is duplicate product ownership.

**Migration**: Use SiYuan core Agent to reason/chat and call SiYuanMemo MCP tools for learning data, card writes, Review context, and UI navigation.

#### Scenario: Old workbench entrypoint is requested
- **WHEN** Browser, Review, Settings, tab, or dialog code previously opened the plugin AI Workbench
- **THEN** that entrypoint is absent or returns explicit unsupported/unavailable behavior instead of opening plugin-owned AI UI

### Requirement: Plugin-owned AI chat/provider/session runtime
**Reason**: Provider orchestration, prompt contracts, tool-loop execution, and AI session persistence belong to the host Agent direction rather than SiYuanMemo.

**Migration**: Keep only MCP-facing typed tool contracts in SiYuanMemo; host Agent supplies reasoning and any generated candidate content.

#### Scenario: Runtime service is reachable only from retired AI paths
- **WHEN** `AIWorkbench*`, `AIChat*`, `AIPrompt*`, `AIBackendSessionService`, plugin `LLMPort`, or AI session-store code has no non-retired active caller
- **THEN** it is deleted or quarantined outside the active runtime path with no compatibility shim
