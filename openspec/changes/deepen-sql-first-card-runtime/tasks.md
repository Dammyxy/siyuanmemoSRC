...
- [ ] 1.1 Add or update Browser deck tests proving SQL page, matched IDs, row hydration order, stats, and source-existence semantics.
- [ ] 1.2 Add or update queue projection tests proving readiness, snapshot rows, row hydration, counters, and incomplete hydration fail-closed behavior.
...
- [ ] 2.1 Introduce a deep Browser card-universe read Module that owns SQL expressibility, page reads, matched IDs, row hydration, stats, source-existence filtering, and unavailable diagnostics.
- [ ] 2.2 Wire `BrowserApplicationService` deck paths through the new Module while preserving existing application Interface methods.
- [ ] 2.3 Move Browser datasource/query-session callers to consume the SQL-first behavior without encoding fallback or full-snapshot rules.
- [ ] 2.4 Update Browser tests to assert active SQL-first paths do not scan all cards for normal page and hydrate requests.

## 3. Queue Projection Runtime

- [ ] 3.1 Introduce a queue projection read Module that owns Queue Projection Readiness, projection identity, rows, cards, counters, and refresh-required/unavailable results.
- [ ] 3.2 Wire `UnifiedDataSourceManager` and `BaseReviewQueue` projection reads through the new Module.
- [ ] 3.3 Update Browser Queue View Lifecycle to consume readiness and attach datasource decisions without duplicating projection state logic.
- [ ] 3.4 Update Review queue tests to prove projection-backed queues fail closed on missing rows/cards and local queue reads remain explicit diagnostics.
...
- [ ] 7.3 Update `ARCHITECTURE.md` with SQL-first card runtime ownership and active path diagrams.
- [ ] 7.4 Update `docs/DDD_RESCAN_BACKLOG.md` with completed slices and any deliberately deferred debt.

## 8. Validation

- [ ] 8.1 Run targeted Browser, queue projection, NeuralRoam, review mutation, Xiuyuan, and SQL repository tests for changed slices.
- [ ] 8.2 Run `node scripts/check-hidden-fallbacks.cjs`.
- [ ] 8.3 Run `pnpm run check:boundaries`.
- [ ] 8.4 Run `pnpm build`.
