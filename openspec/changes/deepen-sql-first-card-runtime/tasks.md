...
- [x] 1.4 Add or update review mutation tests around projection invalidation or patch results for the first selected SQL-first mutation slice.
- [x] 1.5 Add or update Xiuyuan repository tests for read parity by ID, block ID, and aggregate invariants.
...
- [x] 6.1 Introduce a Xiuyuan SQL persistence adapter for read paths that preserves ADR-004 aggregate semantics.
- [x] 6.2 Wire `XiuyuanRepository` read methods by ID and block ID to SQL-first adapter where available without full store scans.
- [x] 6.3 Add indexed lookup support needed for `findAll` or document why `findAll` remains out of scope for this phase.
- [x] 6.4 Add parity tests proving loaded Xiuyuan aggregates preserve faces, card IDs, template ID, block IDs, ownership metadata, and scheduling links.
...
- [x] 8.1 Run targeted Browser, queue projection, NeuralRoam, review mutation, Xiuyuan, and SQL repository tests for changed slices.
- [x] 8.2 Run `node scripts/check-hidden-fallbacks.cjs`.
- [x] 8.3 Run `pnpm run check:boundaries`.
- [x] 8.4 Run `pnpm build`.
- [x] 8.5 Record any non-blocking build warnings and remaining deferred SQL-first work in the final handoff.
