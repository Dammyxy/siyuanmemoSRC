...
- Runtime behavior: Browser queue grids can attach immediately; row reads/counts may still report refreshing/unavailable until projection read data is available. Projection repair becomes an explicit path rather than an implicit side effect of readiness.
