...
export class FilterGroupQueue extends BaseReviewQueue {
    ...
    public async removeCard(cardId: string): Promise<void> {
        try {
            ...
            await this.persistManuallyAddedCards();
            console.log(`[FilterGroupQueue] Card ${cardId} removed`);
        } catch (error) {
    ...
    public async handleReview(cardId: string, rating: number): Promise<void> {
        try {
            ...
            if (rating >= 3) {
                ...
                if (newDueDate > now) {
                    await this.removeCard(cardId);
                    console.log(`[FilterGroupQueue] Card ${cardId} reviewed with rating ${rating}, new due date is in future, removed from queue`);
                } else {
                    console.log(`[FilterGroupQueue] Card ${cardId} reviewed with rating ${rating}, kept in queue`);
                }
