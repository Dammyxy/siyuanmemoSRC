...
describe('IncrementalLearningQueue', () => {
    ...
    describe('持久化', () => {
        it('应该在重新创建队列时恢复手动添加的卡片', async () => {
            ...
            // 创建新的队列实例
            const newQueue = new IncrementalLearningQueue(manager);
    ...
    describe('主题卡片支持', () => {
        it('应该支持项目卡片和主题卡片', () => {
            // 这个测试验证队列可以处理两种类型的卡片
            const now = Date.now();
            const itemCard = createTestCard('item-1', now, 50, CardType.Item);
            const topicCard = createTestCard('topic-1', now, 50, CardType.Topic);
            
            const cards = [itemCard, topicCard];
            const sorted = (queue as any).sortByDueDateAndPriority(cards);
            
            // 验证两种类型的卡片都被正确处理
            expect(sorted.length).toBe(2);
            expect(sorted.find((c: FSRSCard) => c.type === CardType.Item)).toBeTruthy();
            expect(sorted.find((c: FSRSCard) => c.type === CardType.Topic)).toBeTruthy();
        });
    });
});
