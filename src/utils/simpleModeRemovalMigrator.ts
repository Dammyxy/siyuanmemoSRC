...
export class SimpleModeRemovalMigrator {
    ...
    static migrate(oldConfig: RiffIntegrationConfig): Omit<RiffIntegrationConfig, 'mode'> {
        console.log('[SimpleModeRemovalMigrator] Migrating from simple mode to advanced mode');
        console.log('[SimpleModeRemovalMigrator] Old config:', oldConfig);
        ...
        if (!newConfig.incrementalSync.enabled) {
            console.log('[SimpleModeRemovalMigrator] Enabling incremental sync for migration');
        ...
        console.log('[SimpleModeRemovalMigrator] New config:', newConfig);
        return newConfig;
    }
...
详情请查看设置面板。`;
            ...
            console.log('[SimpleModeRemovalMigrator] Migration notification displayed');
        }
    }
    ...
    static async triggerMigrationSync(syncService: any): Promise<boolean> {
        try {
            console.log('[SimpleModeRemovalMigrator] Triggering incremental sync for migration');
            const result = await syncService.incrementalSync();
            
            if (result.success) {
                console.log('[SimpleModeRemovalMigrator] ✅ Migration sync completed successfully');
                console.log(`[SimpleModeRemovalMigrator] Synced ${result.syncedCount} cards`);
                return true;
            } else {
                console.error('[SimpleModeRemovalMigrator] ❌ Migration sync failed:', result.error);
                return false;
            }
        } catch (error) {
            console.error('[SimpleModeRemovalMigrator] ❌ Migration sync error:', error);
            return false;
        }
    }
    ...
    static async handleMigrationError(error: Error, context: string): Promise<void> {
        ...
        console.error(`[SimpleModeRemovalMigrator] ❌ Migration error in ${context}:`, error);
        console.error('[SimpleModeRemovalMigrator] Stack trace:', error.stack);
    ...
    static async performMigration(
        ...
        if (!wasSimpleMode) {
            console.log('[SimpleModeRemovalMigrator] No migration needed');
