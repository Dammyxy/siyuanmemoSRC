...
describe('FilterGroupQueue - 集成测试：评分操作', () => {
  let queue: FilterGroupQueue;

  beforeEach(async () => {
    // Create queue with two groups
    queue = new FilterGroupQueue([
      { id: 'group-1', weight: 1 },
      { id: 'group-2', weight: 1 },
    ]);
  ...
  describe('评分 1 - 重来', () => {
    it('应该将卡片旋转到队列末尾', async () => {
      // Arrange: 创建测试队列状态
      const items = [
        createTestQueueItem({ cardID: 'card-1', blockID: 'block-1', deckID: 'group-1' }),
        createTestQueueItem({ cardID: 'card-2', blockID: 'block-2', deckID: 'group-1' }),
        createTestQueueItem({ cardID: 'card-3', blockID: 'block-3', deckID: 'group-1' }),
      ];
    ...
    it('应该在单卡片队列中保持队列大小为 1', async () => {
      // Arrange: 单卡片队列
      const item = createTestQueueItem({ cardID: 'card-only', blockID: 'block-only', deckID: 'group-1' });
      ...
      await queue.onFeedback(card, { action: 'rate', rating: 1 });
  ...
  describe('混合评分场景', () => {
    it('应该正确处理连续的不同评分操作', async () => {
      ...
      const items = [
        ...
        createTestQueueItem({ cardID: 'card-2', blockID: 'block-2', deckID: 'group-1' }),
        createTestQueueItem({ cardID: 'card-3', blockID: 'block-3', deckID: 'group-1' }),
        createTestQueueItem({ cardID: 'card-4', blockID: 'block-4', deckID: 'group-1' }),
      ];
    ...
    it('应该正确处理多次旋转同一张卡片', async () => {
      ...
      const card1_first = await queue.next();
      ...
      const card1_second = await queue.next();
