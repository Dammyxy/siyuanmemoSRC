 ...
 * - Unified FSRS scheduling
 */
export class IncrementalLearningQueueV2 implements IQueueStrategy<QueueItem> {
  ...
  async next(): Promise<QueueItem | null> {
    console.log('[IncrementalLearningQueueV2] next() called');
    await this._ensureRiffLoaded();

    // 合并本地卡片和 Riff 卡片
    const allItems = [...this.localBuffer, ...this.riffBuffer];

    console.log('[IncrementalLearningQueueV2] next():', {
    ...
    console.log('[IncrementalLearningQueueV2] next() returning card:', {
  ...
  async addItems(items: QueueItem[]): Promise<number> {
    console.log('[IncrementalLearningQueueV2] addItems called with', items.length, 'items');
    console.log('[IncrementalLearningQueueV2] Input items:', items.map(i => ({
      cardID: i.cardID,
      blockID: i.blockID,
      deckID: i.deckID,
    })));
    ...
    console.log('[IncrementalLearningQueueV2] Normalized items:', normalizedItems.map(i => ({
      cardID: i.cardID,
      blockID: i.blockID,
      deckID: i.deckID,
    })));
    ...
    if (newItems.length < normalizedItems.length) {
      console.log('[IncrementalLearningQueueV2] Filtered out', normalizedItems.length - newItems.length, 'duplicate items');
    }
    ...
    console.log('[IncrementalLearningQueueV2] Added', newItems.length, 'items to local queue');
    console.log('[IncrementalLearningQueueV2] Local buffer size:', this.localBuffer.length);
  ...
  async reorder(orderedItems: QueueItem[]): Promise<boolean> {
    try {
      ...
      if (orderedItems.length !== allItems.length) {
        console.error('[IncrementalLearningQueueV2] reorder - Count mismatch');
        return false;
      }
      ...
      console.error('[IncrementalLearningQueueV2] reorder failed:', err);
      return false;
    }
  }
  ...
  private _loadLocalQueue(): void {
    ...
    console.log('[IncrementalLearningQueueV2] Loaded local queue:', {
      deckID: this.deckID,
  ...
  private async _ensureRiffLoaded(): Promise<void> {
    ...
    console.log('[IncrementalLearningQueueV2] Loading Riff cards for deck:', this.deckID);
    ...
    console.log('[IncrementalLearningQueueV2] Riff cards loaded:', {
      deckID: this.deckID,
