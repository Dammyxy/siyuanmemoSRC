 ...
 */
export class GroupDataSource extends ObservableDataSource<QueueItem> {
  ...
  async add(items: QueueItem[]): Promise<Result<number>> {
    ...
    try {
      ...
      if (addedCount > 0) {
        ...
        await this.save();
        // Notify observers after successful addition
        this.notifyObservers();
      }

      return ok(addedCount);
    } catch (error) {
      console.error('[GroupDataSource] Failed to add items:', error);
  ...
  async remove(items: QueueItem[]): Promise<Result<number>> {
    try {
      ...
      if (removeSet.size === 0) return ok(0);
      ...
      if (removedCount > 0) {
        await this.save();
        // Notify observers after successful removal
        this.notifyObservers();
      }

      return ok(removedCount);
    } catch (error) {
      console.error('[GroupDataSource] Failed to remove items:', error);
      return err(error as Error);
    }
  }
