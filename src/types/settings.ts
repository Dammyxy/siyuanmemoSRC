    ...
    skills: {
        conceptCoach: {
            baseRun: [
                ...
                '我会给你一个概念、一张卡片，或一段解释该概念的材料。你的任务不是只给出定义，而是帮助我建立结构化理解，并把关键内容转成可用于自测的高质量问答。',
                '默认按这组用户画像组织输出：已有水平=略懂；目标=理解概念；输出深度=标准。除非材料或上下文明确要求更浅或更深，否则按这组默认档位控制解释粒度。',
                '总原则：不要只做百科定义复述；优先提取边界、关系、作用、适用场景、常见误解；不是每个视角都必须平均展开，若某个视角当前不关键，请明确写“此视角当前不关键”，不要硬凑。',
                '材料没说清楚的地方写“材料未说明”或“这里有不确定性”，不要脑补。优先帮助用户形成可迁移的理解，而不是只记住术语。',
                '所有自测题尽量满足：一题只测一个点，问法具体，答案短、稳定、可重复，需要回忆，不能靠题面直接猜出来。宁缺毋滥，不要为了凑数量输出弱卡。',
                '用清楚、具体、少术语的中文。一旦出现抽象表述，立刻补一个具体例子。目标不是“看懂”，而是下次能想起来、分得清、用得上。',
            ].join('\n'),
            tabs: {
                'working-definition': {
                    run: [
                        '阶段：工作定义。',
                        '先用 1-2 句话给出概念的工作定义：不追求百科式完整，追求抓住本质、便于理解和使用。',
                    ].join('\n'),
                ...
                perspectives: {
                    run: [
                        ...
                        '特性和倾向：回答“什么让它之所以是它”、哪些描述总是/通常/有时/几乎不会成立，并给出关键特性、易误判特性和至少一个具体例子。',
                        '辨析异同：回答它最容易和哪些概念混淆；写出相同点、不同点，以及一个快速区分线索。',
                        '部分和整体：补充典型实例、重要子概念、上位概念，并说明它在整体中的位置或比喻。',
                        '因果关系：说明主要作用、作用背后的原因、适用/不适用场景，以及一个常见错误用法。',
                        '意义和影响：说明为什么值得理解、会改变什么判断或行为，并给出以后什么触发场景该想起它。',
                    ].join('\n'),
                ...
                'integrated-understanding': {
                    run: [
                        ...
                        '其中 essence 尽量压缩成约 100-150 字的整合理解；notWhat 用来说明它不是什么；capabilities 用来说明学会后应该能做到的 3 件事。',
                    ].join('\n'),
                ...
                'self-test-cards': {
                    run: [
                        '阶段：生成高质量自测问答卡。',
                        '把前面的理解转成 10-18 个模式无关的 canonical 自测草稿；如果材料不足，宁可少一些，也不要凑弱卡。',
                        '每张卡只测试一个点，优先测试区别、因果、应用、反例、边界、触发，少做纯定义复述题。',
                        'prompt 要短、明确、需要回忆；answer 尽量控制在 3-20 个字，保持短、稳定、可重复。',
                        'details 默认留空；只有 answer 单独不足以稳定判分时，才补 1-2 条极短补充。summary 只写简短识别语，不要写成长解释。',
                        '在材料允许时，至少覆盖：2 张辨析题、2 张因果题、2 张应用题、1 张反例题、1 张触发题。',
                        '合适时优先使用“为什么……？”“……和……有什么不同？”“什么时候该用……而不是……？”“哪一种情况最能说明……？”“看到……时，我应该想到什么？”',
                    ].join('\n'),
                ...
                'real-world-triggers': {
                    run: [
                        '阶段：现实触发器。',
                        '输出 3 个以后遇到什么情况时应该想起这个概念的触发场景。',
                        '触发器要具体、可识别、能迁移，并能说明这个概念为什么会改变现实中的观察、判断或行动。',
                    ].join('\n'),
