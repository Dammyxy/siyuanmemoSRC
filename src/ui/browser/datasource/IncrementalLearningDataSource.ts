...
type FsrsPluginLike = {
  incrementalQueue?: IncrementalQueueLike;
};
...
export class IncrementalLearningDataSource implements ICardDataSource {
  ...
  constructor(plugin: FsrsPluginLike | undefined, options?: IncrementalLearningDataSourceOptions) {
    this.plugin = plugin;
    this.options = options || {};
  }

  async fetchRows(params: { sortModel: SortModel[]; filterModel: any }): Promise<{ rows: BrowserCard[]; totalCount: number }> {
    const q = this.plugin?.incrementalQueue;

    // ✅ 优先使用 getAllCards()（包含 Riff 卡片）
    let items: any[] = [];
    if (q && typeof q.getAllCards === 'function') {
      // 使用新的 getAllCards() 方法（包含 Riff 卡片）
      items = await q.getAllCards();
    } else {
      // 回退到旧方法（向后兼容）
      items = q?.getAllItems?.() || [];
    }
