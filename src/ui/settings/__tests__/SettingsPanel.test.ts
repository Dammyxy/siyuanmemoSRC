  ...
  return mount(SettingsPanel, {
    props: {
      ...
      i18n: {
        ...
        reviewSourceBlockRefreshEnabled: 'Refresh review source blocks live (advanced)',
        reviewSourceBlockRefreshEnabledHint: 'Off by default. When enabled, multiple review surfaces share one ws-main listener.',
...
describe('SettingsPanel', () => {
  ...
  it('saves the default review open-mode UI toggles from the review tab', async () => {
    ...
    const sourceRefreshItem = formItems.find((item) => item.text().includes('Refresh review source blocks live (advanced)'));
    ...
    const sourceRefreshToggle = sourceRefreshItem?.find('input[type="checkbox"]');
    ...
    expect(sourceRefreshToggle).toBeDefined();
    ...
    await sourceRefreshToggle!.setValue(true);
    ...
    expect(payload.ui.reviewSourceBlockRefreshEnabled).toBe(true);
