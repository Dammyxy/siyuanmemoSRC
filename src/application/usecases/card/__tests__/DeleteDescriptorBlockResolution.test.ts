...
function createDescriptorXiuyuan() {
  const conceptBlockId = '20260105000001-concept';
  const descriptorBlockId = '20260105000002-descrpt';

  const xiuyuan = must(
    Xiuyuan.create({
      blockIDs: [must(BlockId.create(conceptBlockId)), must(BlockId.create(descriptorBlockId))],
      templateID: must(TemplateId.create('builtin-concept-descriptor')),
      faces: [
        must(
          CardFace.create({
  ...
  return { xiuyuan, cardId: card.getId().getValue(), conceptBlockId, descriptorBlockId };
}
