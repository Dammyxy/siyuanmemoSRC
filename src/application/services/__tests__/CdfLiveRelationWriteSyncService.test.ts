...
import {
  ...
  type CdfLiveRelationWriteSyncManagerPort,
...
): CdfLiveRelationWriteSyncManagerPort {
  return {
...
describe('CdfLiveRelationWriteSyncService', () => {
  it('creates missing live relation cards in editor write sync flows with new FSRS state', async () => {
    const manager = createManager();
    const creator = {
      createCards: vi.fn(async () => undefined),
    };
    const service = new CdfLiveRelationWriteSyncService({
      manager,
      cardCreator: creator,
      now: () => NOW,
    ...
    const result = await service.reconcileWriteSync({
  ...
  it('uses draft source markdown for write-sync previews without persisting cards or metadata', async () => {
  ...
  it('keeps concept document block as relation identity instead of concept simple card id', async () => {
    ...
    await service.reconcileWriteSync({
