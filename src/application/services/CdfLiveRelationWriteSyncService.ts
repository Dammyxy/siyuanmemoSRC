...
export interface CdfLiveRelationWriteSyncManagerPort {
...
export type CdfLiveRelationWriteSyncScope = 'source' | 'block-edit';

export interface CdfLiveRelationWriteSyncSourceLoadOptions {
  reconciliationScope?: CdfLiveRelationWriteSyncScope;
  changedBlockId?: string;
}

export interface CdfLiveRelationWriteSyncSourceLoader {
  loadSourceTree(
    sourceBlockId: string,
    options?: CdfLiveRelationWriteSyncSourceLoadOptions,
  ): Promise<CdfLiveBlockNode | CdfLiveBlockNode[] | null>;
}

export interface CdfLiveRelationWriteSyncServiceDeps {
  manager: CdfLiveRelationWriteSyncManagerPort;
  cardCreator: CdfLiveRelationCardCreatorPort;
  sourceLoader?: CdfLiveRelationWriteSyncSourceLoader | null;
  now?: () => number;
  idFactory?: (relation: CdfLiveRelationCandidate) => string;
  xiuyuanIdFactory?: (relation: CdfLiveRelationCandidate) => string;
}

export interface CdfLiveRelationWriteSyncOptions {
  sourceBlockId?: string;
  changedBlockId?: string;
  reconciliationScope?: CdfLiveRelationWriteSyncScope;
...
export interface CdfLiveRelationWriteSyncResult {
  attempted: boolean;
...
export class CdfLiveRelationWriteSyncService {
  private readonly sourceLoader: CdfLiveRelationWriteSyncSourceLoader | null;
  ...
  constructor(private readonly deps: CdfLiveRelationWriteSyncServiceDeps) {
  ...
  private result(
    ...
    updatedCards: FSRSCard[],
    derivedRelationCount: number,
    reason: CdfLiveRelationWriteSyncResult['reason'],
