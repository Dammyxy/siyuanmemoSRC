...
const FULL_RUN_CONTRACT: AIPromptContractDescriptor = {
  ...
  title: 'AI 理解与制卡结构化规则',
  summary: '系统会固定要求首轮返回 workingDefinition / perspectives / integratedUnderstanding / selfTestCards / cdfStructure / realWorldTriggers 这组 JSON。',
  runtimeLines: [
    '你会收到一个 JSON payload，其中至少包含 language、skillId、tabIds、contextSignature、context.currentCard、context.selectedBlocks、context.queueProgress、context.neuralBatch 和 attachedContexts。',
    '严格以当前卡片和当前材料为锚点；超出材料直接支持的地方，必须明确说明“这是补充理解，不是材料原文直接说明”。',
    '只返回合法 JSON，不要附带 Markdown 代码块。',
    'JSON 顶层字段固定为 workingDefinition、perspectives、integratedUnderstanding、selfTestCards、cdfStructure、realWorldTriggers。',
    ...
    'cdfStructure 必须包含 anchors 数组；允许返回多个概念锚点。',
    '每个 anchor 固定包含 id、conceptName、selected、definitionCandidates、descriptorGroups。',
    'definitionCandidates 必须是数组，每项固定包含 id、text、selected。',
    'descriptorGroups 必须是数组，每项固定包含 id、title、selected、items；items 里的每项固定包含 id、text、selected。',
    '当某个 descriptorGroups 项下 items 超过 1 个时，每个 items[].text 都必须写成“提示→答案”，例如“前身→恒星”“时期→恒星演化末期”；仍然只返回 text 字符串，不要新增 cue 字段。',
    '不要在 cdfStructure 中返回 ::: / ;;; markdown；只返回语义 JSON。',
...
const TAB_CONTRACTS: Record<AIConceptCoachTabId, AIPromptContractDescriptor> = {
  ...
  'cdf-structure': {
    settingKey: 'conceptCoach',
    title: 'CDF 语义结构规则',
    summary: '当前 tab 局部重跑时只返回 cdfStructure，其中包含多个概念锚点及其定义候选、描述维度组和条目。',
    runtimeLines: [
      '只返回合法 JSON，不要附带 Markdown 代码块。',
      'JSON 顶层字段固定为 cdfStructure。',
      'cdfStructure 必须包含 anchors 数组；允许返回 0-N 个概念锚点。',
      '每个 anchor 固定包含 id、conceptName、selected、definitionCandidates、descriptorGroups。',
      'definitionCandidates 必须是数组，每项固定包含 id、text、selected。',
      'descriptorGroups 必须是数组，每项固定包含 id、title、selected、items；items 每项固定包含 id、text、selected。',
      '当某个 descriptorGroups 项下 items 超过 1 个时，每个 items[].text 都必须写成“提示→答案”，例如“前身→恒星”“时期→恒星演化末期”；只有单条 item 可以保持纯答案文本。',
      '如果当前材料不足以支撑某个概念的描述维度，可以返回空数组，但不要省略 key。',
      '不要返回 ::: / ;;; markdown；请直接返回语义 JSON。',
      '最小合法示例：{"cdfStructure":{"anchors":[{"id":"anchor-1","conceptName":"概念","selected":true,"definitionCandidates":[{"id":"def-1","text":"定义候选","selected":true}],"descriptorGroups":[{"id":"group-1","title":"作用","selected":true,"items":[{"id":"item-1","text":"带来什么变化","selected":true}]},{"id":"group-2","title":"演化线索","selected":true,"items":[{"id":"item-2","text":"前身→恒星","selected":true},{"id":"item-3","text":"阶段→恒星演化末期","selected":true}]}]}]}}。',
    ],
  },
  'real-world-triggers': {
    settingKey: 'conceptCoach',
