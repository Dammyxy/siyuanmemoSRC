...
const FULL_RUN_CONTRACT: AIPromptContractDescriptor = {
  ...
  runtimeLines: [
    ...
    'descriptorGroups.items[].text 在需要额外正面提示时，可以直接写成“提示→答案”，例如“前身→恒星”；仍然只返回 text 字符串，不要新增 cue 字段。',
...
const TAB_CONTRACTS: Record<AIConceptCoachTabId, AIPromptContractDescriptor> = {
  ...
  'cdf-structure': {
    ...
    runtimeLines: [
      ...
      '同一维度下如果多个子项共享同一正面而需要额外提示，items[].text 可以写成“提示→答案”，例如“前身→恒星”“时期→恒星演化末期”。',
      '如果当前材料不足以支撑某个概念的描述维度，可以返回空数组，但不要省略 key。',
      '不要返回 ::: / ;;; markdown；请直接返回语义 JSON。',
      '最小合法示例：{"cdfStructure":{"anchors":[{"id":"anchor-1","conceptName":"概念","selected":true,"definitionCandidates":[{"id":"def-1","text":"定义候选","selected":true}],"descriptorGroups":[{"id":"group-1","title":"作用","selected":true,"items":[{"id":"item-1","text":"带来什么变化","selected":true}]},{"id":"group-2","title":"演化线索","selected":true,"items":[{"id":"item-2","text":"前身→恒星","selected":true}]}]}]}}。',
    ],
  },
  'real-world-triggers': {
    settingKey: 'conceptCoach',
