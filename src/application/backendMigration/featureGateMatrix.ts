...
export const BACKEND_FEATURE_GATE_MATRIX: BackendFeatureGateRow[] = [
  ...
  },
  {
    ...
    status: 'retained',
    retentionReason: 'backend execute ownership is the release path; flag is retained only as an emergency rollback control',
    reviewAfter: '2026-08-01',
  ...
  },
  {
    ...
    status: 'retained',
    retentionReason: 'live writer/follower smoke passed; flag remains as rollback control for host transaction regressions',
    reviewAfter: '2026-08-01',
    rollbackMode: 'return-unavailable',
    owner: 'backend-worker',
    defaultEnabled: true,
  },
  {
    ...
    status: 'retained',
    retentionReason: 'private API status/diagnostic/read and narrow mutation routing passed live smoke; flag remains as rollback control',
    reviewAfter: '2026-08-01',
    rollbackMode: 'disable-feature-flag',
    owner: 'writer-relay',
    defaultEnabled: true,
