...
export const BACKEND_MIGRATION_OWNERSHIP_MAP: MigratedStateFamily[] = [
  ...
  {
    familyId: 'xiuyuan.sync',
    currentOwner: 'application-command',
    targetOwner: 'backend-worker',
    ownerRuntime: 'backend-worker',
    contract: 'xiuyuan.sync.execute',
    writerRelayPolicy: 'required-for-formal-mutation',
    kernelProxyDependency: 'riff-read-audit',
    ...
    retainedEffects: [
      {
        ...
        kind: 'renderer-host-effect',
        owner: 'infrastructure.siyuan.XiuyuanSyncSiyuanAdapter',
        reason: 'native Riff facts still require SiYuan/kernel read access until kernel Riff proxy is proven live',
        removalCondition: 'kernel Riff read/audit proxy smoke passes and Worker planner consumes typed native Riff facts',
        validation: 'malformed native Riff input, blacklist filtering, kernel proxy unavailable, and partial rollback tests pass',
      },
    ],
    rollbackMode: 'return-unavailable',
    diagnostics: ['commandId', 'syncScope', 'progress', 'unavailableClass'],
  },
  {
  ...
  {
    familyId: 'review.riff-feedback',
    currentOwner: 'application-command',
    targetOwner: 'backend-worker',
    ownerRuntime: 'backend-worker',
    contract: 'review.riffFeedback.execute',
    writerRelayPolicy: 'required-for-formal-mutation',
    kernelProxyDependency: 'riff-read-audit',
    idempotencyKey: 'commandId/cardId/sessionId/ratingOrSkip',
    storage: 'siyuanmemo.db',
    allowedReaders: ['ui.review.v2.sessions.FinalDrillV2Session', 'worker.bootstrap.BackendKernel'],
    allowedWriters: ['backend-worker'],
    compatibilityReads: [],
    featureGate: BACKEND_MIGRATION_FEATURE_GATES.autocardExecuteRelay,
    rolloutFlag: BACKEND_MIGRATION_FEATURE_GATES.autocardExecuteRelay,
    fallbackRemovalCondition: 'FinalDrillV2Session direct native Riff feedback calls are replaced by backend/writer command calls',
    retainedEffects: [
      {
        id: 'final-drill-native-riff-call',
        kind: 'local-fallback',
        owner: 'ui.review.v2.sessions.FinalDrillV2Session',
        reason: 'FinalDrill Riff feedback migration is not cut over yet',
        removalCondition: 'review.riffFeedback.execute handles rating/skip and direct native Riff feedback calls are boundary violations',
        validation: 'rating, skip, kernel proxy unavailable, writer unavailable, and no-local-fallback tests pass',
      },
    ],
    rollbackMode: 'return-unavailable',
    diagnostics: ['commandId', 'cardId', 'queueImpact', 'unavailableClass'],
  },
  {
