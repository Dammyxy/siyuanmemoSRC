...
export const BACKEND_MIGRATION_OWNERSHIP_MAP: MigratedStateFamily[] = [
  ...
  {
    familyId: 'ai.tool-job',
    currentOwner: 'backend-worker',
    targetOwner: 'backend-worker',
    ownerRuntime: 'backend-worker',
    contract: 'ai.tool.job.*',
    writerRelayPolicy: 'required-for-formal-mutation',
    kernelProxyDependency: 'network-proxy',
    idempotencyKey: 'jobId/sessionId/idempotencyKey',
    storage: 'siyuanmemo.db',
    allowedReaders: ['application.services.AIWorkbenchService', 'worker.bootstrap.BackendKernel'],
    allowedWriters: ['backend-worker'],
    compatibilityReads: [],
    featureGate: BACKEND_MIGRATION_FEATURE_GATES.autocardExecuteRelay,
    rolloutFlag: 'AI backend runtime policy plus backend worker gate',
    fallbackRemovalCondition: 'AI tool writes use backend job phases and writer relay commit; UI remains approval/progress/cancel only',
    retainedEffects: [
      {
        id: 'ai-approval-facts',
        kind: 'renderer-host-effect',
        owner: 'ui.ai.AiWorkbenchPane',
        reason: 'approval, rejection, and cancellation are user-visible UI facts',
        removalCondition: 'backend AI job owns lifecycle after UI returns approval fact',
        validation: 'approval rejected, duplicate job id, provider failure, writer unavailable, and content-safe diagnostics tests pass',
      },
    ],
    rollbackMode: 'return-unavailable',
    diagnostics: ['jobId', 'sessionId', 'providerClass', 'status', 'unavailableClass'],
  },
  {
