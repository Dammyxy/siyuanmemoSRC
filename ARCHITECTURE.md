...
- 应用层 `XiuyuanSyncService`、sync planner/apply/input/blacklist/startup helpers、重复 sync types、sync status UI，以及 worker `xiuyuan.sync.execute` contracts/client/catalog/planner/runtime/read-audit host effect/checkpoint apply family 已物理删除。只剩 legacy checkpoint/blacklist/delete-sync 持久字段等待 5.6；不得恢复为 active runtime fallback。
...
- `riffSyncState`、`riffBlacklist` 与 `deleteSync` 目前仅为迁移兼容读取字段，运行时不再推进 checkpoint、写 blacklist 或发起删除；任务 5.6 在迁移读取不再需要后删除这些字段。
