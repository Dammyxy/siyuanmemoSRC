...
- Backend runtime hotspot foundation（2026-05-24）：`packages/contracts/src/backend-rpc.ts` 定义共享 `hotspot.command.submit` / `hotspot.job.get` envelope，以及 Browser aggregate snapshot/page/focus 与 `graph.query` placeholder contracts；`SrsBackendClient` 是 application 唯一 typed caller；`BackendKernel` 通过 `BackendHotspotCommandRuntime` 记录 command state、idempotency replay、pending/terminal/timeout/cancel/unavailable counters 与 latency counters。Browser aggregate 和 graph placeholder RPC 在未实现业务 read model 前返回 typed unavailable，不落成 `METHOD_NOT_FOUND` 或 renderer-local fallback。`writerRelayCommandDispatcher` 已允许 follower 提交 `hotspot.command.submit`，`kernel-rpc` relay method allowlist 同步开放该方法。Kernel companion capability diagnostics 现在区分 `kernelNetworkProxy`、`kernelNetworkSse`、`privateHttp`、`privateSse` 与 `riffReadAuditProxy`；kernel-sidecar Riff read/audit proxy 仍是 not-configured/unavailable，不能把 approved renderer host-effect 误报成 kernel-sidecar 能力。
- Xiuyuan sync backend apply slice（2026-05-24）：`xiuyuan.sync.execute` 是 Backend Worker typed RPC，由 `SrsBackendClient.executeXiuyuanSync()` 调用；`BackendKernel` 委托 `WorkerXiuyuanSyncPlanner` 从 `siyuanmemo.db` 读取本地 Xiuyuan/card facts，并通过 `BrowserSrsBackendWorkerTransport` 的 `siyuan.riff.readAudit` host effect 调用组合根批准的 `XiuyuanSyncSiyuanAdapter` 读取 native Riff card facts。dry-run/audit 仍只返回 audit-safe plan，diagnostics 不包含卡片正文；非 dry-run 现在通过 `WorkerSqliteDatabaseService.applyXiuyuanSyncPlan()` 在 Backend Worker DB authority 内按 update/delete/create 顺序写入 `xiuyuans`、`cards` 与 tombstones，并返回 `applyImpact.reason='applied'` 和 changed ids。`XiuyuanSyncService` 的生产 full/incremental UI 入口暂仍是原 application facade，直到后续任务把 entrypoint 切到 backend command facade 并补齐 native Riff write/attr relay smoke。
