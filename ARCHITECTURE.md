...
- Kernel companion / backend worker / AI kernel streaming / SiYuan Agent tool fast paths
...
- SiYuan Agent tools：上游 `H:/project-F/flashcard/siyuan` 的 `origin/dev` 暴露 `Plugin.addAgentAction({ name, description, handler })` 与 kernel `siyuan.mcp.registerTool(name, config, handler)` / `unregisterTool(name)`；Go 侧会把插件 MCP handler 的返回值 `JSON.stringify` 后包成 text content，因此 `src/kernel.ts` 的 handler 直接返回 SiYuanMemo typed envelope，不自行返回 `{ content, isError }`。Kernel 只注册 `memo_query` / `memo_card` / `memo_review` / `memo_ui`、要求非空 `action`、拒绝 unknown/blocked action，并通过 `writer.submitCommand(method='agent.tool.execute')` 转发到 active writer；它不读写 `siyuanmemo.db`，不拥有卡片、队列、Review、CDF、scheduler 或 AI 业务。`ApplicationContext` 装配 `AgentToolService` 作为 application facade，并注入 `AgentCardDraftService` 使用现有 `LLMPort`、AI settings 与 `AISiyuanPort` 读源：`memo_query` 只读 learning overview；`memo_card draft` 先把 `sourceContent` / block id / editor focused-selected blocks 归一为 bounded source packets，再生成 RemNote-style preview-only 候选（稳定 draft id、默认 5、最大 20、不写卡，action 非 safe 但非持久 mutation）；AI/settings/source/JSON 不可用时返回 typed unavailable 或 validation，不回退本地 heuristic。`save/create/suspend/resume` 才走 Card CRUD writer path；`memo_review` 只提供 explanation/hint/source/score suggestion，拒绝 feedback/grade/commit；`memo_ui` 通过 frontend `addAgentAction` 在 `ApplicationContext` ready 后注册并走 Dialog/Tab manager。缺 MCP/API/frontend/writer/read owner 时返回 typed unavailable，不安装 shim，不做 hidden fallback。
...
- `src/application/services/AgentToolService.ts`：SiYuan Agent tool application facade；把 `memo_query`、`memo_card`、`memo_review`、`memo_ui` 路由到现有 Browser/Card/Review/Dialog/Tab owners，`memo_card draft` 只委托 `AgentCardDraftService`，保持 draft preview 与 selected save 分离，拒绝 raw SQL/workspace dump/review feedback commit，并对大结果做 bounded projection。
- `src/application/services/AgentCardDraftService.ts`：SiYuan Agent card draft application use case；负责 explicit/editor source packet bounding、RemNote-style structured prompt、`LLMPort.chat({ responseFormat: 'json_object' })`、AI JSON parsing、supported type validation、stable draft id、preview-only candidate normalization，以及 AI/source/settings unavailable 的 typed result。
...
- `src/application/commands/writerRelayCommandDispatcher.ts`：writer relay command dispatcher，集中 `review.feedback`、queue projection、NeuralRoam、AutoCard、private API、Semantic、AI session 与 `agent.tool.execute` relay method 的 typed backend/application hook 调用；unsupported method 显式 `BACKEND_UNAVAILABLE`。
