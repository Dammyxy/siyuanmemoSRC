...
9. Browser neural 子视图的 route bar、trace/list projection、jump/focus/source/anchor/history commands、engine/navigation/bookmark/review-surface handoff commands，以及 route/trace refresh、enrichment、convergence、preview controller state 由 `src/ui/browser/neural/*` helpers 维护；route selector/create/rename/delete/save temporary/switch 都通过 `NeuralRoamSessionQueue` contract 进入 queue/application path，Browser 只显示当前全局 active route 的 concept pool、station pool 与 paged route log，不直接访问 SQL。Browser 发起 route switch/create/delete 且已有 NeuralRoam Review surface 时，会通过 `TabManager.hasOpenNeuralReviewTab()` / `DialogManager.hasOpenNeuralReviewDialog()` 探针先确认 reset，再切换全局 `activeRouteId` 并复用现有 review-surface handoff 同步打开的 NeuralRoam Review。
   - `航线日志` 是 route-owned paged log，读 `getRouteHistoryPage()` / SQL route history，只记录整条 active route 的 node trajectory；`双链轨道` 属于 Orbit/Hyperspace engine-local history/trace，不作为 Browser route-log 页签名称，也不用于重建已清空的 route log。
