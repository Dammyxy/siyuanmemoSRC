...
- `SrsTransparencyApplicationService` / `SrsEditorDialog.vue` / `ReviewView.vue` 只展示轻量分歧提示、学习曲线 evidence facts 和透明度事实；learning-curve evidence 只读 bounded review history 并输出 `ready / insufficient-data / low-quality-data / unavailable` display diagnostics，不触发 UI SQL、queue projection、backend/kernel RPC、scheduler commit 或参数写入；只有 `arena.srs.advisoryOnly === false` 且样本数达到 `minimumReviewsForConfidence` 时才允许进入实验写入路径，默认不接管正式 due
