...
- Xiuyuan sync backend read-plan slice（2026-05-24）：`xiuyuan.sync.execute` 是 Backend Worker typed RPC，由 `SrsBackendClient.executeXiuyuanSync()` 调用；`BackendKernel` 委托 `WorkerXiuyuanSyncPlanner` 从 `siyuanmemo.db` 读取本地 Xiuyuan/card facts，并通过 `BrowserSrsBackendWorkerTransport` 的 `siyuan.riff.readAudit` host effect 调用组合根批准的 `XiuyuanSyncSiyuanAdapter` 读取 native Riff card facts。该 slice 只做 audit-safe normalize/plan：diagnostics 不包含卡片正文，full 模式可计划 managed Riff local delete，incremental/audit 不计划破坏性删除；`dryRun=false` 仍返回 `apply-not-implemented`，不写 `cards`、`xiuyuans`、tombstones、attrs 或 native Riff。`XiuyuanSyncService` 的生产 full/incremental 写入口暂仍是原 writer-owned application path，直到后续任务把 apply ordering 和 UI entrypoint 切到 backend/writer relay。
