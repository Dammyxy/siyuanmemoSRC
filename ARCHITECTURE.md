...
- `src/application/services/UnifiedDataSourceManager.ts`：统一队列创建、缓存、失效、观察者通知中心；RetrievalPractice / IncrementalLearning 的 projection snapshot / rowsByIds 读取通过 `SrsBackendClient` 收口到 `readQueueProjectionSnapshot()` 与 `getQueueProjectionCardsBySnapshotIds()`，非 projection rollout 队列返回显式空结果并保持原策略读。
