...
- `Phase 5` 已推进到第四安全步（双 action 消费链）：worker `kernel.transaction.ingest` 支持事务 envelope inbox（`idempotencyKey` 去重、队列/事务 backpressure 显式 unavailable、`diagnostics.status.ingest` 可观测计数），并通过 `kernel.transaction.dequeue` 输出 action 队列；应用层在 `VITE_SIYUANMEMO_ENABLE_KERNEL_TRANSACTION_INGEST` 开启时启用 `KernelTransactionIngestHandler + KernelTransactionActionPump`，writer 模式直写/直读 worker，follower 模式统一走 kernel writer relay；当前已收敛两条 action：`native-riff-remove -> XiuyuanSyncService.handleNativeRiffRemove` 与 `native-riff-upsert -> XiuyuanSyncService.handleNativeRiffUpsert`（fallback 到 incremental sync）。AutoCard 其余决策仍待继续下沉。
