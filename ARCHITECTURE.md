...
- Xiuyuan sync backend apply slice（等待任务 5.5 删除）：`xiuyuan.sync.execute`、`SrsBackendClient.executeXiuyuanSync()`、`BackendXiuyuanSyncRuntime -> WorkerXiuyuanSyncPlanner`、`siyuan.riff.readAudit` host effect、`WorkerXiuyuanSyncRuntime` 和 sync checkpoint persistence 仍存在于 worker/client contract，但应用层已无生产入口或生命周期 caller。该 family 当前是孤立死路径，不是 Native Riff import/adoption owner；5.5 必须同时删除 contracts、client/catalog、worker planner/runtime、host effect、checkpoint behavior 与 obsolete worker tests，不能重新连接到 Browser、Review、startup 或 transaction。
...
- 应用层 `XiuyuanSyncService`、sync planner/apply/input/blacklist/startup helpers、重复 sync types 与 sync status UI 已物理删除。只剩 worker `xiuyuan.sync.execute` family 等待 5.5，以及 legacy checkpoint/blacklist/delete-sync 持久字段等待 5.6；两者都不得恢复为 active runtime fallback。
- `custom-xiuyuan-id` / `custom-fsrs-xiuyuan-id` 已降级为旧数据兼容兜底读取来源，不再作为自动同步里的真相源，也不再对 managed Riff Xiuyuan 做后台写入、自修复清理或删除时清空。
- 不存在 `plugin-start`、`browser-open`、Review lifecycle、transaction 或 timer Native Riff 同步触发器；`KernelTransactionActionPump` 对旧队列中的 Native Riff action 只做无副作用消费，直至任务 5.5 删除旧 action/RPC catalog。
