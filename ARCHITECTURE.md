...
- SiYuanMemo-owned SRS / Native Riff import cutover：普通 quick-card、block-menu、AutoCard、Progressive、Topic-derived、NeuralRoam concept entry 的 SRS authority 是 SiYuanMemo 本地 Xiuyuan/FSRS persistence。Browser 维护菜单把 Native Riff 操作拆成三条显式路径：`NativeRiffImportModule` 只读预览并导入缺失 semantic faces，`NativeRiffAdoptionModule` 原位接管旧 `riff-managed` 卡并从 live block Markdown 重建渲染语义，`SrsCardSemanticsRepairService` 单独修复现有 `local-owned` 卡。`NativeRiffImportSourceAdapter` 不暴露 add/remove/rating；导入 receipt 只表示 provenance，不授予 Native Riff ownership。插件启动、Browser、Review、transaction fanout、card create/delete 与 settings 均不再装配或调用连续同步。`XiuyuanSyncService`、sync helper runtimes、应用层 Riff blacklist Module、startup-sync background-work kind、sync status UI、`xiuyuan.sync.execute` contracts/client/catalog/worker planner/runtime、Riff read-audit host effect 与 sync checkpoint apply behavior 已删除。`riffIntegration` / `deleteSync` 设置、canonical store 的 `riffBlacklist` / `riffSyncState` 字段与新库 `riff_sync` schema 也已删除；`SqliteMigrationService` 只在一次性 retirement migration 中兼容读取旧 file-store blacklist / SQLite `riff_sync`，转换成 `native-riff-import-exclusion` tombstone 后删除旧表并写 migration receipt。AutoCard、Progressive、Topic-derived 与普通 Xiuyuan 创建只保存本地卡和发布领域事件；Card CRUD 删除只改变 SiYuanMemo 本地状态；Review rating/skip 只更新本地 Review/冲刺状态，FinalDrill 只维护本地冲刺队列和进度。不得恢复连续同步 fallback。
...
- Browser projection/count work under Review pressure：Browser warmup 仍由 `SRSBrowser -> browserQueueProjectionWarmupRuntime -> BrowserApplicationService.ensureQueueReadModelReady -> UnifiedDataSourceManager.readQueueProjection({ type: 'readiness' })` 拥有，属于 **BrowserProjectionIndex** derived read-model preparation，不属于 Review answer authority。BrowserProjectionIndex owns matched row identity、counts、filter/sort projections、projection warmup/repair 和 row hydration；projection stale / refreshing / unavailable 只能进入 Browser diagnostics 或 projection-maintenance diagnostics，不能让 active Review session 等它选下一张。recoverable repair 只能由 `BrowserApplicationService.repairQueueReadModel -> UnifiedDataSourceManager.repairQueueProjection(...)` 显式提交。Browser sidebar counts 通过 `BrowserQueueCountReadModel` 读取 projection/session count evidence；除 NeuralRoam route-owned `getSize()` 外，普通 count refresh 不再调用 `manager.getQueue(queueType)` 创建 Queue Module。`DialogManager.getActiveReviewQueueType()` 与 `TabManager.getActiveReviewQueueType()` 只暴露只读 Review surface pressure；Browser runtime 消费该 signal 后，在 Review active 时只立即 warm/count 当前可见/Review-relevant queue，sidebar projection-backed queues 延迟 750ms 并按 queue coalesce targeted warmup/retry。非活动队列的 count/readiness/repairable `projection_stale` / `missing_derived_cache` repair 在 Review pressure 下不抢占 Review hot path；Browser visible queue 仍 explicit `ready/refreshing/unavailable`、fail closed，不使用 stale snapshot fallback。`onQueueReady` 仍只按 affected queue type 刷新 counts，NeuralRoam 不进入 projection warmup。
...
- Kernel transaction queue：`kernel.transaction.ingest/dequeue/requeue` 的 queue state、dedupe key、snapshot restore/persist、backpressure counters 与 diagnostics 由 `worker/kernel-transaction/WorkerKernelTransactionRuntime.ts` owns。transaction fan-out 只为 AutoCard 候选生成 action；renderer `TransactionWebSocketService` 与 backend worker 都从 raw transactions 加短期 provenance snapshot 计算同一 plan，worker 不信任 renderer 序列化 plan。`WorkerKernelTransactionRuntime` 只把 plan 转为 `auto-card-candidates` queue items 并做 dequeue coalescing，不执行 AutoCard side effects、Native Riff、Xiuyuan sync、doc-tree refresh 或 Review truth side effects。`WorkerSqliteDatabaseService` 只创建 runtime、传入 `SqliteFileServiceAdapter` 与 queue limit options，并保留原 RPC facade method / diagnostics shape；snapshot 文件名与 public backend RPC method string 不变。这个 runtime 不接管 scheduler、Review truth、queue projection、AI/Job/Hotspot 或 agent 行为。
...
- 装配 `XiuyuanApplicationService`；SQL active 时 `ApplicationContext` 给 `XiuyuanRepository` 注入 `SqlXiuyuanReadRepository`，`findById()` 读 `xiuyuans` 主键，`findByBlockId()` 通过 `cards.block_id + cards.xiuyuan_id` 索引 join 到 `xiuyuans`，再用 `cards.dto_json` 恢复 ADR-004 aggregate card links；这两个读路径不扫描 `UnifiedStorageManager.getAllXiuYuans()`。`findAll()` 仍是显式管理/迁移面的全量枚举，暂不纳入本阶段 SQL-first active path。所有 `XiuyuanRepository` read methods 只能 hydrate domain aggregate，不能顺手持久化 cardIds/faces 修复；修复必须通过显式 write usecase 或 import/adoption Module 进入写路径。`XiuyuanApplicationService` 的修缘写入 usecases 共享组合根注入的 `XiuyuanSiyuanAdapter`；组合根不再创建连续同步服务、sync lock、blacklist service 或 Browser sync status observer。
...
- Backend runtime hotspot foundation（2026-05-24，Browser/Graph/Review read-write slice 2026-07-10 更新）：`packages/contracts/src/backend-rpc.ts` 定义 Browser deck page、aggregate snapshot/page/focus、`graph.query`、`review.sourceRefresh.execute` 等 active contracts；`SrsBackendClient` 是 application 唯一 typed caller。Browser aggregate read 现在由 `WorkerBrowserAggregateReadService` 在 Backend Worker 内基于 `siyuanmemo.db` 生成 snapshot id / generation / queryFingerprint，并提供 page/focus/source-existence/hierarchy facts；deck datasource lifecycle 的普通首屏/翻页 rows 首选 `BrowserApplicationService.getDeckPage()` / backend `browser.deck.page`，aggregate page/snapshot 只给显式 full-universe action 或 diagnostics 使用并携带 action reason，不再把普通 deck page 绑定到 full matched-id snapshot。`graph.query` 现在由 `WorkerGraphQueryService` 返回 presentation-ready nodes/edges、limit/continuation 和 content-safe diagnostics；缺 graph adapter、missing/unreadable node、deadline/invalid request 均返回 typed unavailable/partial/failed，不执行 renderer fallback。`writerRelayCommandDispatcher` 允许 follower 提交 domain sync、Progressive/Topic command 与 Review source-refresh command；Native Riff feedback、sync 与 read-audit relay 已删除。Kernel companion capability diagnostics 只保留当前网络/private API 能力，不再声明 `riffReadAuditProxy`，`kernel.js` 也不绑定 `riff.read` / `riff.audit`。Graph 的 `resolveNeuralGraphQuery` host effect 仍按 ownership map 归档为 migration-only adapter，直到 approved kernel/private graph read adapter 接管。
- Xiuyuan continuous-sync backend slice 已退役：contracts 不再声明 `xiuyuan.sync.execute` 或 Xiuyuan sync/Riff read-audit DTO，client/catalog/worker registry 不再暴露该 method，renderer-worker protocol 不再允许 `siyuan.riff.readAudit`，SQLite service 不再提供 sync local-facts/apply/checkpoint facade。显式 `NativeRiffImportModule` / `NativeRiffAdoptionModule` 走独立只读 source port 与本地 persistence，不复用 backend sync family。
- Progressive / Topic-derived backend command cutover（2026-05-25，Native Riff retirement 2026-07-10 更新）：`progressive.command.execute` 与 `topic-derived.command.execute` 是 Backend Worker typed RPC，由 `SrsBackendClient.executeProgressiveCommand()` / `executeTopicDerivedCommand()` 调用。`BackendKernel` 负责 command idempotency replay、typed unavailable、terminal rollback/audit result shape，并通过 `BrowserSrsBackendWorkerTransport` host-effect bridge 回到组合根批准的 application executor；`ProgressiveReadingService` / `TopicDerivedItemService` 的公开写入口在 backend client 可用时只提交 backend command，不再先提交 generic hotspot accepted 再本地继续写。当前 renderer-only materialization facts 仍由 review/block-menu/selection runtime 在调用前收集为 bounded input；Worker owns command state/result/idempotency，writer 窗口只执行 SiYuan doc/card/attr 与 SiYuanMemo 本地 persistence side effects，不注册或修改 Native Riff。follower window 会通过 `FollowerCommandClient -> writerRelayCommandDispatcher -> SrsBackendClient` relay `progressive.command.execute` / `topic-derived.command.execute`，缺 writer relay 时显式 `WRITER_UNAVAILABLE`，不使用 follower-local mutation fallback。
- Review low-priority hotspot slice（2026-07-10）：FinalDrill rating/skip 只维护本地冲刺队列、进度与 SiYuanMemo Review 状态；`ReviewSiyuanPort.reviewRiffCard()` / `skipReviewRiffCard()`、`ReviewApplicationService.executeFinalDrillRiffFeedback()` 与 backend/writer `review.riffFeedback.execute` 已删除。Review source refresh 继续通过 `ReviewApplicationService.executeReviewSourceRefresh()` 获取 backend-authored `refresh-required | no-op | missing-source | unavailable | failed` impact，再刷新可见内容；backend missing-source impact 可更新 source-existence projection 并返回 `cleanupMissingSource`，不做 UI-local inference。
...
9. SQL active 的队列候选真相是 `cards` 当前状态；`queue_state` 只保存筛选配置、临时黑名单、手动加入、session 排除和手动顺序等 overlay。手动加入卡解析按 card id / block id 定点查询，查不到才清理无效 manual entry，不再常规回退到无过滤全量 `getCards()`。`queue_projection_*` 表作为 backend/writer owned derived storage 存在，用来记录可重建的队列 rows、counters、invalidations、generation、policy hash 与显式 rebuild/repair 命令状态。
   ...
   `SrsBackendClient` 保留 `queue.projection.snapshot` / `queue.projection.rowsByIds` / `queue.projection.replace` transport；`UnifiedDataSourceManager` 对 application caller 只暴露 Queue Projection Lifecycle：`readQueueProjection`、`repairQueueProjection`、`observeQueueProjection`。`BaseReviewQueue`、Browser query kernel 和 `UnifiedQueueStrategy` 通过 typed lifecycle read 读取 projection rows/counters、按 projection row id hydrate 卡片，并在 ordinary feedback 后 hot patch 或按 refresh-required 请求显式 repair。`QueueProjectionLifecycle.read` 的 readiness/snapshot/rows-by-id/diagnostics 都是 passive read path：只返回 ready/refreshing/unavailable/null/[] 与 diagnostics，不调用 `queue.getCards()` 或 `queue.projection.replace`。只有 `QueueProjectionLifecycle.repair` 才能从真实 queue `getCards()` 物化 ordered rows，再经 backend/writer `queue.projection.replace` 写入 projection storage；follower 模式必须经 writer relay 提交，不允许 follower-local writer bypass。writer relay materialization 成功后，runtime 会把这次 writer 返回的 rows/counters 作为同 generation 的短期 materialization echo 用于当前 snapshot 与 row hydration；echo 在队列失效、全量失效或正式 review commit 后清除，避免 follower 本地 worker 旧 DB 把刚写成功的 projection 读回 unavailable。

   `QueueProjectionRuntime` 也是 projection live identity event owner：materialized / refreshed ready identity 只在可读 policy/generation 成立后发布，invalidated / echo-cleared 只发布 recheck signal，payload 不含 rows/cards/source markdown/review feedback/scheduler decision；`UnifiedDataSourceManager.observeQueueProjection()` 暴露 typed subscription，不新增 backend/kernel broadcast。projection-backed 队列若 snapshot / row hydration / counter / explicit repair 依赖不可用，会显式 `QUEUE_PROJECTION_UNAVAILABLE` 或 `BACKEND_UNAVAILABLE`，不再悄悄回退 strategy rows、size methods 或本地计数。`QueueProjectionRuntime` 显式报告 `existing-queue-strategy` / `parity-checking` / `backend-projection` / `backend-advance` / `advance-contract-unavailable` / `projection-unavailable`，`UnifiedDataSourceManager.getQueueProjectionRolloutDiagnostics()` 只保留公开 façade；NeuralRoam 只有在 backend advance 可用时才是 review-ready；`getQueueProjectionRolloutState()` 只作为显式 rollback/parity override，不引入 UI direct SQL/RPC 或 follower-local writer bypass。
...
- `ReviewCommitUseCase` 是 kernel 下层的 backend feedback adapter，只保留 card read、runtime policy、backend `review.feedback`、writer relay、scheduler config 与 Arena 批次记录依赖；旧本地 scheduler / review log / transaction runner 构造依赖已移除。worker 内通过 `WorkerReviewCardMutationPersistenceModule` 在现有 `runTransaction('review.feedback')` transaction owner 下提交 Card Schedule Store、Review Ledger、domain-sync evidence、Review Transaction Undo Journal、mutation stamp 与 delta/checkpoint durability。该 transaction 返回 durable receipt 后，`WorkerReviewFeedbackRuntime` 才从 receipt 构造或排队 queue projection impact；projection maintenance failure 只返回 `deferred` / `refresh-required` / `unavailable` evidence，不回滚已提交 Review truth。`ReviewCommitUseCase` 是 `queueImpact -> ReviewProjectionReceipt` 的唯一 application mapper，`ReviewAttemptKernel` 只透传 receipt 并补 attempt diagnostics。
- `worker/review/SrsReviewKernel.ts` 是 worker-owned Review answer seam：canonical Interface 仅以 `execute(command)` / `read(query)` 暴露 start、answer、skip、undo、current 与 diagnostics，不保留第二组 public session methods。`answer` command 返回分离的 authoritative session state 与 normalized receipt：receipt 只暴露 commit outcome、updated card、稳定 idempotency/fact identity、durability summary、undo token/evidence、queue impact 和 typed diagnostics，不泄漏 scheduler/SQL transaction stages，也不导入 application-owned Review Content Target。`WorkerReviewSessionRuntime`、`WorkerReviewFeedbackRuntime` 与 `WorkerReviewCardMutationPersistenceModule` 留在 kernel Implementation 内部；backend `review.session.*` RPC 只做 transport validation、单次 kernel 调用和 typed result mapping。`undo` 消费 Review Transaction Undo Journal evidence，恢复 Card Schedule Store before-state、SessionQueueIndex frontier、lookahead/counters，并追加 `review_events(event_type='review-undo-v1')` reversal evidence，journal 缺失或 stale undo token 显式 fail closed，不落回 renderer history。它是 Anki-style kernel 的收口点：小 Interface 提供高 leverage，大 Implementation 继续隐藏 scheduler、Review Ledger、Card Schedule Store、SessionQueueIndex、NextCardPreparation 与 diagnostics。
...
- `ProgressiveExcerptMaterializer`：摘录 materialization 接口 owner，集中处理选区归一、source lineage、source-child / daily-note / library 落点、attrs、topic-card linkage、duplicate result 与失败回滚；实际 Siyuan 与 SiYuanMemo local-card 写入仍经 application ports 与现有 service helper
...
- `src/application/services/SettingsService.ts` / `ReviewLogService.ts`：配置、日志等横切服务；其中 `ReviewLogService` 只保留 legacy monthly JSON 与 SQL review log 的兼容读投影，`addReviewLog*` / `addDrillLogV2` / `addRescheduleLog` renderer 写口一律返回 `BACKEND_UNAVAILABLE`，正式 Review 写入只能走 backend `review.feedback` / Review truth 命令或未来 writer-owned command；`SettingsService` 在 init/update 时负责把持久化的 `ui.enableDebugLogs` 同步到 SiYuanMemo 运行时 logger 级别，不再 patch 宿主全局 `console`。连续同步专用 `RiffBlacklistService`、legacy blacklist migration application Module/port/adapter 与 `riffIntegration` setting 已删除；旧 blacklist 只由 `SqliteMigrationService` 的一次性 retirement migration 读取并转换为 import exclusion。
...
- `src/application/services/queue-projection/QueueProjectionReadModule.ts` / `QueueProjectionRuntime.ts` / `QueueProjectionBuilder.ts` / `QueueProjectionParityDiagnostics.ts` / `QueueProjectionReadinessService.ts`：`QueueProjectionLifecycle` 是 application external Seam，Interface 只有 `read/repair/observe`；`QueueProjectionRuntime` 是其 deep Implementation，owns projection-readable queue checks、rollout state normalization、readiness service composition、passive snapshot/row reads、explicit materialization、writer-relay replace、short-lived materialized echo cache、unavailable diagnostic recording/clearing 和 rollout diagnostic construction。Builder / parity / readiness 继续分别负责六队列 row materialization、parity harness 与 canonical `ready | refreshing | unavailable` orchestration；RetrievalPractice / IncrementalLearning 复用 core `SrsV2QueuePolicy` 保持现有队列顺序，把当前可见卡、manual outstanding、rotation cards、due bucket、sort key、counter、frontier candidate、普通 feedback affected-set 与 broad invalidation plan 物化为 backend/writer 可持久化事实；FilterGroup / FinalDrill / Leech / NeuralRoam builder 保留共享 projection row contract，把 filter hash/session transfer/preview policy、drill entry/log/FlipElement order、lapse/manual/action retention、neural synthetic/associated-review/history cursor 等事实放入 typed payload metadata；所有 builder source query 必须请求 active-source cards，projection snapshot/rowsByIds hydrate 也会过滤 known missing source cards。NeuralRoam projection 只表达 Browser/count/diagnostic/repair 视图，不表达 advance cursor；Browser 只消费 lifecycle result，不生成 policy、不读 SQL。parity harness 用于比较现有 strategy snapshot rows/counters 与 projection rows/counters，输出缺行、多行、顺序差异与 counter delta；worker `review.feedback` 对 RetrievalPractice / IncrementalLearning 可生成 post-commit projection impact，对非 SRS projection queue 返回 `deferred` 时只排队 backend-owned maintenance。`read` 不调用 `queue.getCards()` 或 `queue.projection.replace`；只有 `repair` 能从 queue domain 生成 rows 并经 writer/backend replace 写入。follower 成功 relay 后只复用本次 writer materialization echo，不落本地 worker DB，不给 UI 写 projection。materialized/refreshed ready identity 会通过 `FrontendInstanceRuntime -> KernelSidecarClient -> queueProjection.publishIdentityChanged` 发出 cross-window identity-only broadcast；其他窗口过滤本地 echo、重复和不完整身份后，交回 `QueueProjectionRuntime.acceptRemoteLiveIdentityEvent()` 并经 `observe` 通知 Browser。
...
- `src/core/card/render-contract/SrsCardRenderContractResolver.ts`：SRS Card Render Contract Module；在语义类型确定后解析 Protyle、quick-symbol、native multi-cloze、image-occlusion、concept、concept-definition、descriptor 的 renderer kind、render family、front/back contract、required receipts、deterministic evidence、render repair patch 和 diagnostics。Browser semantic repair 只消费 deterministic render patch；Review policy、presentation preparer 和 content renderer 只消费 contract，不再维护并行 renderer selection / quick side rules。
...
- `src/application/factories/createAutoCardKernelXiuyuanServiceBundle.ts`：AutoCard/kernel/Xiuyuan bounded-context service factory，集中 Xiuyuan repository/application service 与 AutoCard handler 装配；不创建 Xiuyuan sync service，不注入 Native Riff 写 adapter。
...
- `src/application/usecases/card/*`：卡片 CRUD 用例；Siyuan block text / attrs 与本地删除能力通过 `CardCreationSiyuanPort` / `CardDeletionSiyuanPort` 从 `ApplicationContext` 注入，不在 usecase 内默认构造 infrastructure adapters；删除路径不暴露 Native Riff remove intent。
...
- `src/ui/settings/settingsSavePayload.ts`：Settings 面板 save payload helper；负责保存时的 numeric/list normalization、queue legacy key removal、storage conflict resolution 与 final payload assembly；不会发出 `riffIntegration`。
...
- `src/ui/settings/settingsLoadState.ts`：Settings 面板 props load hydration helper；负责 FSRS/queue/scheduler/Arena/UI 初始状态 normalization、legacy queue spacing key 读取与 storage conflict resolution hydration；不加载 Riff sync trigger。
...
- Native Riff 不属于 Progressive 写边界；只有 Browser 显式 import/adoption 可通过只读 source port 读取候选事实
- 对 capture 落点，经 `ConfiguredCaptureStoragePort`

Progressive 制卡契约：

- split / topic-derived 生成物只创建本地 Xiuyuan / FSRS 卡，不注册原生 Riff；linear split 只立即为当前 active piece 建 Topic 卡，完成当前片后再释放下一片，nonlinear split 则立即为全部 piece 建 Topic 卡。excerpt 前台只创建 SiYuan 摘录实体与 `ExcerptRecord`，Topic card 由 `ProgressiveExcerptCompletionService` 后台补完，前台结果允许没有 `topicCardId`。
...
- 摘录即 Topic：摘录文档、全局摘录库摘录和 Daily Note 摘录块上的后续符号/选区制卡，都会落到本地 derived Item 卡，不注册原生 Riff，也不把块级 card-type 属性当作事实源。
...
- 这些 progressive 卡的类型真相源保存在本地 Xiuyuan / FSRS 数据里，不依赖块级 `custom-fsrs-card-type`；块属性只保留必要的 `custom-xiuyuan-id` 与 `custom-fsrs-reading-*` 来源/lineage 信息，不新增 Native Riff 标记。
- 新的 progressive-owned `piece` / `excerpt` / `derived-item` 不再写 deprecated `custom-fsrs-card-type`，保存 / 更新触达时也会显式 scrub 旧 attr；非 progressive 历史数据只在明确 migration/compat read 边界读取旧属性。
...
- `TransactionWebSocketService`（订阅宿主 `eventBus.on('ws-main')`，不再 monkey-patch 主 `WebSocket.onmessage`；当前只承载 AutoCard、doc tree review scope 与 review source refresh，共享 fan-out plan 不再生成或分发 Native Riff action）
- `NativeRiffImportModule` / `NativeRiffAdoptionModule`（Browser 维护菜单显式触发；前者只读预览并导入缺失 faces，后者原位接管旧 `riff-managed` 数据；两者都不提供 Native Riff add/remove/rating 能力）
- `AutoCardHandler`（由 `ApplicationContext.createAutoCardHandler()` 装配本地 AutoCard 依赖；候选块队列 -> settled 评估 -> worker-first 决策（可选）-> `AutoCardExecutionRuntime` 执行 envelope（planner-decision/topic-derived）-> `AutoCardPlannerExecutionRuntime` 执行本地 planner side effects；MenuManager one-click scan 复用同一 factory）

主设计原则：

- Browser / Review 刷新优先走事件与统一数据源通知
- 不依赖分散轮询来维持主状态一致性
- WebSocket 与 Siyuan integration 属于 infrastructure / handler 边界；Native Riff 只允许经显式只读 import/adoption Module 进入，不应反向污染 UI 直接调用链
...
当前 Native Riff 只读导入 / Xiuyuan 本地所有权边界补充：
...
- `NativeRiffImportModule` 只通过 `NativeRiffImportSourcePort` 读取候选事实，preview 不写本地或 Native Riff；apply 只创建缺失 semantic faces，并保留不可变 import receipt。
- `NativeRiffAdoptionModule` 只在用户显式 apply 后原位修改现有 `riff-managed` 记录，保留 card/Xiuyuan identity、调度、复习历史、tags 与 priority，并从 live Markdown 重建语义和渲染契约。
- 应用层 `XiuyuanSyncService`、sync planner/apply/input/blacklist/startup helpers、重复 sync types、sync status UI，以及 worker `xiuyuan.sync.execute` contracts/client/catalog/planner/runtime/read-audit host effect/checkpoint apply family 已物理删除。`riffIntegration` / `deleteSync` setting、canonical `riffBlacklist` / `riffSyncState` 字段、新库 `riff_sync` schema 与应用层 blacklist migration Module 也已删除；不得恢复为 active runtime fallback。
- `custom-xiuyuan-id` / `custom-fsrs-xiuyuan-id` 已降级为旧数据兼容兜底读取来源，不再作为自动同步里的真相源，也不再对 managed Riff Xiuyuan 做后台写入、自修复清理或删除时清空。
- 不存在 `plugin-start`、`browser-open`、Review lifecycle、transaction 或 timer Native Riff 同步触发器；backend RPC catalog、worker registry 与 host-effect protocol 也不再保留连续同步入口。
- 插件主动删除固定为本地语义：Browser / Review / Card CRUD 只删除或隐藏 SiYuanMemo 本地 Xiuyuan / CardDTO、清理旧块属性并写持久 tombstone；`CardDeleted / CardsDeleted` 只驱动本地 observer/queue 刷新。
- Card delete command/result、application port、Siyuan adapter、领域事件与 composition root 均不再暴露 Native Riff remove intent、hard-delete authorization、`deletedFromRiff` 或 remove-card endpoint。
- 默认本地删除写入的持久 tombstone 会带 block id、card id、xiuyuan id 与可用的 Native Riff receipt identity；`NativeRiffImportModule` preview 把命中项分类为 tombstoned/excluded，普通 apply 不重建，只有显式 restore-and-import 才移除对应抑制证据。
- local-owned 始终优先：同一 semantic face 已存在本地卡时，普通 import 不覆盖模板、卡面、调度或复习历史；缺失 face 才可创建，旧 `riff-managed` 数据只能通过显式 adoption 原位接管。
- retirement migration 只兼容读取旧 file-store `riffBlacklist` 与旧 SQLite `riff_sync.blacklist`，把有效 block id 写成 durable `native-riff-import-exclusion` tombstone，随后删除 `riff_sync` 并标记 `native-riff-persistence-retirement-v1`；它不恢复 checkpoint、delete-sync 或持续同步行为。
- `UnifiedStorageManager` canonical store 不再持久化 `riffBlacklist` / `riffSyncState`；当前内容只包含本地 Xiuyuan/CardDTO、删除 tombstone 与普通 sync metadata。冲突 hash 和 merge 不再包含 Riff checkpoint/blacklist。
...
- Kernel transaction empty-poll stabilization（2026-07-10 更新）：`KernelTransactionIngestHandler` 成功写入 transaction inbox 后会唤醒 `KernelTransactionActionPump`，writer relay 收到 `kernel.transaction.ingest` 并落入 backend 后也会唤醒本窗口 action pump；`KernelTransactionActionPump` 对连续 `actionCount=0` 的 poll 做 bounded empty backoff，避免普通编辑期间固定空 `dequeue-local/dequeue-relay` 抢占 renderer。真实 ingest/push 活动在没有 empty backoff 时可即时 poll；若刚连续空 poll，activity wake 不会重置 backoff，只等待 bounded 窗口，避免普通 typing transaction storm 反复打空 dequeue。writer relay required / follower relay unavailable / self-relay repair 等 explicit error contract 不变。
- Daily editing jank mitigation（2026-07-10 更新）：transaction classifier 与 `AutoCardHandler.handle()` 都先用 operation payload 做 cheap quick-card marker prefilter；可检查的 insert/update 内容不含 `>>`、`::`、`;;`、cloze 等 marker 时不发出 `auto-card-candidates`，delete 与无法检查内容的 operation 继续保守通过，避免漏掉真实 marker。`KernelTransactionActionPump` 只消费 AutoCard action，不再维护 Native Riff pending、cooldown、timer、retry 或同步调用。
- Shared transaction hot path（2026-07-10 更新）：`TransactionWebSocketService` 对每个 `ws-main` transaction batch 只做一次共享分类，抽取 changed block ids、AutoCard marker/maybe-scan、doc-tree 与 review-source dependency hint，并只记录 content-free counters/spans；Native Riff marker、dispatch reason、action 与 diagnostics 均已删除。`KernelTransactionIngestHandler`、`AutoCardHandler`、`DocTreeReviewScopeService` 通过 consumer predicate 跳过普通无关编辑。Review source refresh 继续走共享 coordinator：每个复习 surface 只订阅自己的 source dependency，保留 self-edit suppression、native Protyle edit skip、advance-pending drop、quiet debounce 与 in-flight dirty follow-up。
- `Phase 5` transaction queue 当前只承载 `auto-card-candidates`：worker `kernel.transaction.ingest` 支持事务 envelope inbox、稳定 idempotency、多窗口去重、背压、dequeue/requeue 与本地快照恢复；action queue 不再声明、解码、合并或恢复 `native-riff-remove` / `native-riff-upsert`。应用层 `KernelTransactionActionPump` 只负责 AutoCard handoff；Native Riff 连续同步不再拥有 transaction 入口。
...
- One-click symbol batch path（2026-07-10 更新）：用户手动的一键符号制卡走文档级 batch 主链：`DocumentPostCreationScanService` 先收集 accepted candidates，再由 `AutoCardHandler.executeBatchFromBackend()` 通过 `autocard.executeBatch` 进入 backend worker；`XiuyuanApplicationService.createFromBlocksBatch()` 委托 `CreateXiuyuanFromBlocksUseCase.executeBatch()`，`finalizeXiuyuanCreationBatch()` 一次性 `saveMany` 聚合本地持久化成功，并通过 `CardsCreatedEvent` 把批量 card/block/xiuyuan id 交给 `ReviewScopeCardCreationSyncService` / `UnifiedDataSourceManager` 做一次 queue refresh/invalidation。该链路不注册或修改 Native Riff。
- P6 ownership closure（2026-07-10 更新）：`BackendKernel` 新增 `p6.ownership.query` / `p6.ownership.command` handlers；读类 ownership report 明确返回 `compatibility-read`，side-effect ownership report 明确返回 `writer-relay`，都带 `diagnosticEventId`，不再让 P6 owner 查询落成 `METHOD_NOT_FOUND`。旧 Phase 6 direct boundary debt 已从 application 层移走：Xiuyuan scanners/usecases、AutoCard scanner、BlockMenu、DialogManager、DataAccessFacade、BlockAttr cleanup、DocumentPostCreation scan 等不再直接 `siyuanApi.sql(...)`，统一通过 `HostBlockQueryPort` 与 `HostBlockQuerySiyuanAdapter` 获取语义化 host block reads；Progressive / Topic-derived 需要的块属性名进入 `BlockAttrContract` / `ProgressiveAttrContract`，application 层不再 import `@/core/siyuan/block`。Native Riff continuous-sync trigger/attr ownership 已退役；更宽的 Xiuyuan / Progressive / Topic-derived backend-worker command ownership 仍是 staged write scope，当前写 owner 是 writer relay。
...
- Review Entry Target：application Seam 以 `ReviewEntryTarget` discriminated Module 分开 projection queue、managed queue、static subset 与 NeuralRoam launch identity。topbar、Browser、block menu、Semantic temporary Review、NeuralRoam 与 compatibility ingress 先由 `ReviewEntryTargetResolver` 解析；只有 projection queue 进入 `ReviewAdmissionModule`，其余 target 不携带 projection admission 字段。Admission Ticket 绑定 target identity、entry surface、policy hash 与 generation，stale/ambiguous/unsupported evidence fail closed；Entry Target 不携带 render/content 字段。

- Review Content Target：application Seam 现在以 `ReviewContentTarget` discriminated Module 表达 standard card、Topic-derived item、progressive excerpt、source location；`LegacyReviewContentTargetAdapter` 是唯一 raw card/progressive metadata ingress，负责 canonical identity、SiYuan/Xiuyuan content authority、processing/scheduling classification、render intent、actions、version evidence 和 typed unavailable。`UnifiedReviewAdapter -> ReviewRenderableContext -> ReviewContent/ReviewView` 只消费已解析 target；missing/detached/conflicting/unsupported evidence fail closed。SRS Card Render Contract 的 presentation consumer 通过 typed target 读取合同，不在 UI 重新推断 target kind。current edit、source navigation、answer、convert/advance、skip/back 均先构造 typed target command；target 保持 read-only，不拥有 question/answer 副本或 queue projection content truth。

- Queue Projection Lifecycle：`UnifiedDataSourceManager` 的外部 Interface 收敛为 `readQueueProjection(request)`、`repairQueueProjection(command)`、`observeQueueProjection(listener)`。`read` 覆盖 readiness/snapshot/rows-by-id/diagnostics 且永不触发 materialize；`repair` 才能 materialize/rebuild/refresh/invalidate，并经 backend worker 或 writer relay 提交；`observe` 只发布 canonical ready/invalidated identity。旧 readiness/snapshot/hydration/materialization/subscription public methods 已删除。echo、single-flight、deletion filtering、policy/generation normalization 与 rollout diagnostics 留在 `QueueProjectionRuntime` Implementation 内。

- Bounded-context Runtime Access：`ApplicationContext` 继续拥有 startup/disposal/runtime policy，但 Review、Browser/Queue、Progressive、integration caller 只接收 `ReviewRuntimeAccess`、`BrowserQueueRuntimeAccess`、`ProgressiveRuntimeAccess`、`IntegrationRuntimeAccess` 的显式 typed members。bootstrap cycle 使用 bind-once callback port；before-bind、double-bind、disposed access 都显式失败。没有 generic string lookup、通用 DI container 或长期 getter/contextRef 双路径。

- Kernel Companion Background Work lifecycle：P0 已在 application/backend-client 层加入 `KernelCompanionBackgroundWorkRegistry`，把长维护任务收口为 `submit/status/cancel/defer/shutdown` 生命周期 Module，而不是把 DB / scheduler / msgpack truth 写权迁进 `kernel.js`。`SrsBackendClient.schedulePendingReviewTruthFlush()` 对 pending SQL truth rows 提交 `review-truth-backfill` job；job 调用既有 backend Review truth owner 执行 `review.truth.backfill` 并记录 state、attempt、error、counter diagnostics。`SrsBackendClient.flushReviewTruthBeforeUnload()` 只做 bounded quick flush，并先 shutdown registry，使 queued/running backfill 显式 deferred/canceled，不会在 unload 路径启动 heavy work；`dispose()` 同样 shutdown registry、清 timer/queued flush，并阻止 in-flight pressure/error 结果重新 arm timer。`KernelTransactionActionPump` 消费同一个 application registry：polling 不再拥有裸 `setInterval` 生命周期，而是提交 `kernel-transaction-action-polling` job，只保留 dequeue/requeue、writer relay 与 AutoCard handoff；Native Riff upsert/remove action 不再调用 Hybrid/Xiuyuan sync。`dispose()` cancel active polling job，并让 late dequeue 结果不再派生新后台 work。Xiuyuan Native Riff startup jobs 已从 composition root 移除；registry 不再接收 `xiuyuan-startup-sync` submission。P0 仍不能接管 scheduler、Riff/card writes、msgpack truth 或 SQLite DB ownership。
