...
- 初始化 `siyuanmemo.db` 的 sql.js 持久化层；首次启动先把旧 `unified-cards.msgpack`、`queues.msgpack`、月度 review logs 与 `arena/store.json` 迁入 SQL，迁移失败以 `STORAGE_UNAVAILABLE` fail closed，不再允许 env 触发 legacy storage rollback；SQL active 后 DB 以二进制文件写入，旧 base64 envelope 只作为读取兼容与迁移备份；`UnifiedStoragePersistence` 只保留旧 unified store loader 供初始迁移使用，不再暴露旧 msgpack save callback，`check-no-runtime-msgpack.cjs` 防止 production runtime 新增 msgpack 读写入口
