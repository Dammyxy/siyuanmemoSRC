...
- `src/application/services/AIWorkbenchService.ts`：通用 AI chat runtime 与 concept-coach 结构化 renderer 的状态和动作编排；session record scaffold/build/apply projection、persist scheduling、prompt/request runtime 与 self-test target/draft runtime 已下沉到 focused helpers。
