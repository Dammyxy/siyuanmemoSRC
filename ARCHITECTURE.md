...
- 装配 `CardApplicationService` / `BrowserApplicationService` / `ReviewApplicationService`；其中 `ReviewApplicationService` 的 `ReviewSiyuanPort` 与 `CardContentQueryService` 的 `QuerySiyuanPort` 由组合根注入，不在 service 内默认构造基础设施 adapter
