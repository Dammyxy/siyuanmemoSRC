...
- `worker/review/WorkerReviewFeedbackRuntime.ts` / `worker/review/ReviewJournalProjectionReconciler.ts`：`WorkerReviewFeedbackRuntime` owns queue mode / commit policy validation、SchedulerRouter commit、`review_events` append、review commit idempotency enforcement 和 queue projection impact/delta；`ReviewJournalProjectionReconciler` owns restart-time Review journal projection entry normalization、durable `review_events` evidence matching、SRS queue-type filtering、staleness detection 和 derived queue projection replacement。`SqliteDatabaseService` 仍是 SQL worker startup / transaction authority，只注入 journal store、queue projection、repository、durable event lookup 和 transaction Adapter。
