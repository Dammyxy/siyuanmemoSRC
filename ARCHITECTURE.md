...
- Browser queue read model：Browser 队列视图的 pageable/searchable/sortable rows 由 `BrowserQueueViewModule -> BrowserApplicationService.ensureQueueReadModelReady()/getQueueQuerySnapshot()/getQueueRowsByIds() -> QueueBrowserQueryKernel` 读取。该路径从 queue card facts 与 Browser query filters 计算行，不再把 Review `queue.getSnapshotRows()`/projection rows 当作 Browser row authority；`searchText/docId/scopeDocIds/cardType/sort` 是 Browser query-time 参数，不进入 Review projection policy identity。
