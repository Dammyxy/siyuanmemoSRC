...
- Review feedback latency diagnostics：`review.session.feedback` slow summary 必须保留 `session-feedback-total`，同时把 worker 内部耗时拆到更窄 session step。Session preflight、commit、advance、undo-journal append、state shaping 分别进入现有 timing span；普通路径不新增 info log，只在既有 slow-summary / runtime timing surface 里暴露。SQLite delta append 写 `open.msgpack`、`sealed-*.msgpack`、manifest JSON 时携带 `purpose='sqlite-delta.append'` 与 `substep='write-open-segment' | 'write-sealed-segment' | 'write-manifest'`，避免预期 durable append 在 host breakdown 里退化成 `purpose=unknown substep=unknown`。
