...
- Phase 4 stable writer lease（2026-05-06）：writer lease 默认 TTL 统一提升到 60 秒（`FrontendInstanceRuntime`、旧 `KernelWriterLeaseGuard`、`kernel.js` capability 与 `.env.example`），降低 SiYuan 多窗口/后台 renderer 定时器节流导致的 writer 误过期；`FrontendInstanceRuntime` 在 follower startup/manual/visibility/heartbeat refresh 都先执行 `writer.getLease` 观察 active holder，已有别的 writer 时保持 follower 且不主动 `writer.acquireLease`，只有 observe 到空 lease 且窗口可见时才 acquire，避免后开 follower 噪声和抢锁；窗口 `visibilitychange` / `focus` 回到可见时会立即 refresh ownership，因此可见窗口可在 lease 空闲时马上接管；启动新 runtime 前会释放同一 JS context registry 里的全部旧 runtime（即使 `runtimeScopeId` 不同），避免单窗口重复装配留下另一个 active writer。`FrontendInstanceRuntime` 现在会把 `visibilityState`、`documentHasFocus`、`locationHref` 随 `writer.hello/acquireLease/renewLease` 上报到 kernel writer lease；`documentHasFocus` 只作为诊断，不再作为普通可见窗口之间抢 writer 的加分项；`kernel.js` 会拒绝隐藏 requester 抢空 lease；active 普通 SiYuan app/window owner 一旦成功持有 lease，即使 hidden/unfocused 也保持 sticky，不被后开普通窗口抢占；release、TTL 过期、无 active lease、同 instance renew/acquire 仍允许接任/续租；可见普通窗口仍可接管缺少前台诊断的旧 holder 或 `enhance=true` / QuickNote 等辅助 holder，辅助窗口不能反抢普通 app writer。writer lease payload 现在带 `leaseEpoch` 与 `ownerChangedAt`；只有 holder 真实变更才递增 `leaseEpoch`，续租只更新 heartbeat/expiry，避免用相同 `acquiredAt` 误判双 writer。
