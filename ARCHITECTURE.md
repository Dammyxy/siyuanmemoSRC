...
- `Phase 5` 已推进到第三安全步（首条消费链）：worker `kernel.transaction.ingest` 支持事务 envelope inbox（`idempotencyKey` 去重、队列/事务 backpressure 显式 unavailable、`diagnostics.status.ingest` 可观测计数），并新增 `kernel.transaction.dequeue` action 队列；应用层新增可灰度启用的 ws-main transaction ingestion handler 与 `KernelTransactionActionPump`（`VITE_SIYUANMEMO_ENABLE_KERNEL_TRANSACTION_INGEST`），writer 模式直写/直读 worker，follower 模式统一走 kernel writer relay；当前首条 action 仅收敛 `native-riff-remove -> XiuyuanSyncService.handleNativeRiffRemove`，其余 AutoCard/Riff 决策仍待继续下沉。
