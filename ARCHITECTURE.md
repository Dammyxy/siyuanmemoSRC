...
- Backend runtime hotspot foundation（2026-05-24，Browser/Graph read model 2026-05-25 更新）：`packages/contracts/src/backend-rpc.ts` 定义共享 `hotspot.command.submit` / `hotspot.job.get` envelope，以及 Browser aggregate snapshot/page/focus 与 `graph.query` contracts；`SrsBackendClient` 是 application 唯一 typed caller；`BackendKernel` 通过 `BackendHotspotCommandRuntime` 记录 command state、idempotency replay、pending/terminal/timeout/cancel/unavailable counters 与 latency counters。Browser aggregate read 现在由 `WorkerBrowserAggregateReadService` 在 Backend Worker 内基于 `siyuanmemo.db` 生成 snapshot id / generation / queryFingerprint，并提供 page/focus/source-existence/hierarchy facts；deck datasource lifecycle 优先通过 `BrowserApplicationService.getDeckAggregatePage()` / `getDeckAggregateSnapshot()` 消费 backend aggregate，不再把 deck page 首选路径绑定到 renderer-local full snapshot。`graph.query` 现在由 `WorkerGraphQueryService` 返回 presentation-ready nodes/edges、limit/continuation 和 content-safe diagnostics；缺 graph adapter、missing/unreadable node、deadline/invalid request 均返回 typed unavailable/partial/failed，不执行 renderer fallback。`writerRelayCommandDispatcher` 已允许 follower 提交 `hotspot.command.submit`，`kernel-rpc` relay method allowlist 同步开放该方法。Kernel companion capability diagnostics 现在区分 `kernelNetworkProxy`、`kernelNetworkSse`、`privateHttp`、`privateSse` 与 `riffReadAuditProxy`；kernel-sidecar Riff read/audit proxy 仍是 not-configured/unavailable，不能把 approved renderer host-effect 误报成 kernel-sidecar 能力。Graph 的 `resolveNeuralGraphQuery` host effect 仍按 ownership map 归档为 migration-only adapter，直到 approved kernel/private graph read adapter 接管。
