...
10. `sync.conflict.merge` 除了合并 review/card 事实与 invalidating queue projection，还会从 conflict card row 携带 `source_exists=0` 的 missing-source projection；这样另一端已确认删除的有效 block-id 卡不会在本机保持 `source_exists NULL` 并经 source-unchecked fail-open 回到 Review/Browser 队列。
