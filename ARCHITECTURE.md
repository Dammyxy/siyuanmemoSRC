...
- Review session hot path：SRS v2 Review 队列的 `UnifiedQueueStrategy.onFeedback()` 可以返回 `{ status:'advanced', nextItem, counterSnapshot }`，该结果来自 `SrsV2SessionQueueRuntime.answerAndAdvance()` 已经选好的下一张卡。`ReviewSessionController.grade()` 优先直接应用该 `nextItem`，只在非 advanced / 非 SRS 队列路径调用 `queue.next()`。失败、conflict、unavailable 仍保持 fail-closed，不用旧本地 queue snapshot 静默续跑。`queue.projection.replace` 是派生 projection cache 更新，不是外部 conflict source；worker `review.feedback` 自己 durable persist 成功后会保持 persisted main DB fast-skip eligible，除非存在非空 conflict source 或真实 main DB divergence evidence。
