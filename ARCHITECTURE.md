...
- `Phase 4` 已推进到 relay 闭环灰度：kernel companion 除 `writer.hello/getLease/acquireLease/renewLease/releaseLease` 外，新增 `writer.submitCommand/getCommandResult/takeCommand/completeCommand/failCommand`；`FrontendInstanceRuntime` 负责 hello/acquire/renew/release 生命周期并在 writer 模式轮询执行 relay command，`FollowerCommandClient` 负责 follower 提交/等待结果；`ReviewCommitUseCase` 的 worker-first `review.feedback` 与 `BrowserApplicationService` 的 worker-side `browser.sourceExistence.applySweepHost` 写入在 follower 模式都支持 follower->writer relay；composition root relay dispatch 已覆盖当前全部 worker mutation（`review.feedback`、`browser.sourceExistence.applySweepHost/update/applySweep`、`kernel.transaction.ingest/dequeue/requeue`、`autocard.decision.resolve`、`autocard.execute`）；另外 kernel relay queue 在 writer 接管时会把未完成 pending command 重绑定到新 owner，runtime 在 relay 轮询遇到 lease unavailable 时会自动降级 follower，避免多窗口 handover 下命令悬挂；writer relay 不可用时返回 explicit unavailable，不做隐藏 fallback 双写。
...
- P6 closure/Phase 8 foundation（2026-05-01）已补第一轮 owner map 与 private API 基础：新增 `src/application/backendMigration/ownershipMap.ts` 作为迁移 owner/gate 可读清单；`ApplicationContext` 暴露 ownership map 与 feature gates 给运行时编排；contracts 增补 `private.read.*` 与 `private.command.execute` DTO，writer relay method typing 新增 `private.command.execute`；worker `BackendKernel` 新增 `private.health/private.diagnostics.status/private.audit.query/private.read.*` 只读入口，以及 `private.command.execute` 的显式 requestId/idempotencyKey 约束和审计轨迹；`PrivateApiClient` + `PrivateApiService` + `PrivateApiAuditService` 提供 capability 检查、payload size 限制、follower->writer relay routing 与审计记录，不引入 kernel 直接写 DB 路径。
...
- Phase 9 cutover（2026-05-01）已执行首轮旧路径 retirement：新增 `parityHarness`、`featureGateMatrix`、`check-backend-migration-cutover.cjs` 并接入 `pnpm run check:boundaries`；`ReviewCommitUseCase` 已移除本地 review commit 主路径、`SchedulerRouter` 已移除 `route()` 主入口、`BrowserDeckQueryKernel` 已移除 `sql-fallback-getAllCards` 主路径、`AutoCardHandler` 执行已改为 backend-worker ownership（backend 不可用时 explicit unavailable）。R4（2026-05-02）已明确 Browser 读取策略为“保留 compatibility read”：`BrowserApplicationService` 在 backend deck/matched-id/rows/count/stats 调用失败时允许回落 SQL/legacy 快照，并在 owner map + cutover allowlist 中登记 removal condition（后续 `RM020` 才切 backend-only）。
- Runtime policy remediation（2026-05-01）已新增 `src/application/backendMigration/runtimePolicy.ts` 统一解析 backend migration gates/env defaults 与能力位，并在 `ApplicationContext`/`ReviewCommitUseCase`/`AutoCardHandler`/`KernelTransactionIngestHandler`/`KernelTransactionActionPump`/`PrivateApiClient` 消费：当 backend write family 需要 writer relay 但 runtime 不可用时统一 fail-closed（`BACKEND_UNAVAILABLE`/`WRITER_UNAVAILABLE`）；`.env.example` 的 release 示例改为 `backend+writer` 开启，`private API` 与 `AI backend runtime` 继续默认关闭。
