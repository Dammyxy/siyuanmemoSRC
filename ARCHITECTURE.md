...
- `src/application/services/AgentToolService.ts`：SiYuan Agent tool application facade；把 `memo_query`、`memo_card`、`memo_review`、`memo_ui` 路由到现有 Browser/Card/Review/Dialog/Tab owners，保持 draft preview 与 selected save 分离，拒绝 raw SQL/workspace dump/review feedback commit，并对大结果做 bounded projection。
