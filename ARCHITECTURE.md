...
- `Phase 5` 已推进到第五安全步（双 action + 合并消费 + 指标可观测）：worker `kernel.transaction.ingest` 支持事务 envelope inbox（`idempotencyKey` 去重、队列/事务 backpressure 显式 unavailable），并通过 `kernel.transaction.dequeue` 输出 action 队列；应用层在 `VITE_SIYUANMEMO_ENABLE_KERNEL_TRANSACTION_INGEST` 开启时启用 `KernelTransactionIngestHandler + KernelTransactionActionPump`，writer 模式直写/直读 worker，follower 模式统一走 kernel writer relay；当前 action 已收敛 `native-riff-remove` 与 `native-riff-upsert` 两条消费链，并在 pump 侧做“单轮 upsert 合并执行 + remove blockIds 合并去重”；`diagnostics.status.ingest` 已补 action 队列观测字段（queue/enqueued/dequeued/remove/upsert totals）。AutoCard 其余决策仍待继续下沉。
