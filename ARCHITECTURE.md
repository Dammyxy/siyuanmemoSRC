...
- `src/application/services/UnifiedDataSourceManager.ts`：统一队列创建、缓存、失效、观察者通知中心；六个 review queue 的 projection snapshot / rowsByIds 读取都通过 `SrsBackendClient` 收口到 `readQueueProjectionSnapshot()` 与 `getQueueProjectionCardsBySnapshotIds()`，RetrievalPractice / IncrementalLearning / FilterGroup / FinalDrill / Leech 的 review progression 默认为 `backend-projection`，NeuralRoam 的 review progression 默认要求 `backend-advance`；`ensureQueueProjectionReady()` 是 Browser/manager 共享的投影可读性入口，背后委托 `QueueProjectionReadinessService` 计算 canonical policy identity、single-flight materialization、短等待、`refreshing` retry guidance 与 `unavailable` machine cause；组合根可提供 queue projection rollout state 进入 `existing-queue-strategy` / `parity-checking` rollback 模式，backend/projection 失效会被记忆为 `projection-unavailable` 诊断，NeuralRoam 缺 advance capability 会记忆为 `advance-contract-unavailable` 而不是 projection ready。
...
- `src/application/services/queue-projection/QueueProjectionBuilder.ts` / `QueueProjectionParityDiagnostics.ts` / `QueueProjectionReadinessService.ts`：六队列 projection row builder、queue projection parity harness 与 readiness orchestration；RetrievalPractice / IncrementalLearning 复用 core `SrsV2QueuePolicy` 保持现有队列顺序，把当前可见卡、manual outstanding、rotation cards、due bucket、sort key、counter、frontier candidate、普通 feedback affected-set 与 broad invalidation plan 物化为 backend/writer 可持久化事实；FilterGroup / FinalDrill / Leech / NeuralRoam builder 保留共享 projection row contract，把 filter hash/session transfer/preview policy、drill entry/log/FlipElement order、lapse/manual/action retention、neural synthetic/associated-review/history cursor 等事实放入 typed payload metadata；所有 builder source query 必须请求 active-source cards，projection snapshot/rowsByIds hydrate 也会过滤 known missing source cards。NeuralRoam projection 只表达 Browser/count/diagnostic/repair 视图，不表达 advance cursor。readiness service 暴露 `ensureReady()`，把 backend snapshot/materialization 结果映射成 shared `ready | refreshing | unavailable` contract，并 owns policy identity、single-flight、generation-on-commit 与 expected unavailable mapping；Browser 只消费结果，不生成 policy、不 repair、不读 SQL。parity harness 用于比较现有 strategy snapshot rows/counters 与 projection rows/counters，输出缺行、多行、顺序差异与 counter delta；worker `review.feedback` 已复用 projection rows 生成 delta，`queue.projection.snapshot` / `queue.projection.rowsByIds` RPC 暴露 rows/counters/hydration，`queue.projection.replace` 只允许 application manager 为缺失 generation 做显式 materialization 或 writer relay 写入；follower 成功 relay 后只复用本次 writer materialization echo，不落本地 worker DB，不给 UI 写 projection。
