...
Last update: 2026-05-06 (Round 286)
...
| P2 | Phase 7 live kernel-network provider smoke now has raw `network.fetchExternal` and real AI provider prompt evidence, but SSE/delta streaming is still not a completed contract | `src/infrastructure/ai/KernelAINetworkProxyAdapter.ts`, `kernel.js`, `specs/001-backend-migration-next/quickstart.md` | Add `network.openExternalSse` only if the UI needs token delta streaming; keep prompt path on backend job + kernel proxy and continue checking no renderer/raw fetch fallback |
