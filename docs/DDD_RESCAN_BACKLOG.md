...
Last update: 2026-04-30 (Round 226)
...
| P1 | Large active files still mix state, command, projection, and persistence concerns; Browser chrome preference/open-state hydration, neural trace/list projection, neural command/controller helpers, grid snapshot hydration, selection projection, batch action feedback/dialog policy, and toolbar Spread dialog orchestration have been split out; AI workbench result normalization, conversation tree runtime, general-chat tool-loop, prompt/request runtime, CDF/write runtime, session runtime mechanics, thread/message normalization, context signature/card projection, run-status/title projection, AI workbench pane CDF/self-test/message render projection, Review AI sidecar/companion command orchestration, Review more-menu projection, Review neural command helper, Review progressive excerpt command runtime, Review open-as/source refresh/keyboard runtime, Review shell/SRS editor command runtime, SettingsPanel tab/subtab navigation view-model, Settings AI prompt/user-skill view-model, Settings save/default/maintenance state projection, Settings maintenance commands, Settings props load/form projection, Settings form commands, Settings load/save commands, and Settings AI dialog/user-skill commands have also been split out, but larger loading/lifecycle/write-command orchestration seams remain | `AIWorkbenchService.ts`, `SRSBrowser.vue`, `ReviewView.vue`, `SettingsPanel.vue`, `AiWorkbenchPane.vue`, `src/ui/browser/{browserChromePreferences.ts,browserSurfaceState.ts,browserDataSnapshots.ts,browserSelectionScope.ts,browserActionFeedback.ts,browserActionParamDialogs.ts,browserSpreadDialog.ts}`, `src/ui/browser/neural/{neuralTraceViewModel.ts,neuralListViewModels.ts,neuralBrowserCommands.ts,neuralNavigationCommands.ts,useNeuralBrowserController.ts}`, `src/application/services/{AIWorkbenchResultNormalization.ts,AIWorkbenchConversationTreeRuntime.ts,AIWorkbenchGeneralChatRuntime.ts,AIWorkbenchPromptRuntime.ts,AIWorkbenchCdfRuntime.ts,AIWorkbenchSessionRuntime.ts,AIWorkbenchThreadNormalization.ts,AIWorkbenchContextProjection.ts,AIWorkbenchRunProjection.ts}`, `src/ui/ai/aiWorkbenchPaneProjection.ts`, `src/ui/review/v2/{reviewAICommands.ts,reviewMoreMenuItems.ts,reviewNeuralCommands.ts,reviewProgressiveExcerptCommands.ts,reviewOpenAsCommands.ts,reviewShellCommands.ts,reviewSrsEditorCommands.ts,reviewSourceRefreshRuntime.ts,reviewKeyboardRuntime.ts}`, `src/ui/settings/settingsPanelViewModel.ts`, `src/ui/settings/settingsAIViewModel.ts`, `src/ui/settings/settingsSavePayload.ts`, `src/ui/settings/settingsStateDefaults.ts`, `src/ui/settings/settingsMaintenanceViewModel.ts`, `src/ui/settings/settingsMaintenanceCommands.ts`, `src/ui/settings/settingsLoadState.ts`, `src/ui/settings/settingsFormViewModel.ts`, `src/ui/settings/settingsFormCommands.ts`, `src/ui/settings/settingsLoadSaveCommands.ts`, `src/ui/settings/settingsAIDialogs.ts` | Continue splitting only along active behavior boundaries; next safe cut is `ReviewView.vue` Arena conflict/advisory command glue, review filter/scope command runtime, current-content editor bridge, `AiWorkbenchPane.vue` command helpers, or Browser infinite datasource/loadData controller, not naming-only moves |
...
1. Continue splitting the largest active files by state / command / projection / persistence boundaries; Browser chrome preference/open-state hydration, Browser neural projection/command/controller helpers, Browser grid snapshot/selection/action dialog helpers, AI workbench result normalization/tree runtime/general-chat runtime/prompt runtime/CDF runtime/session runtime/thread normalization/context projection/run projection, AI workbench pane projection, Review AI command helper, Review more-menu projection, Review neural command helper, Review progressive excerpt command runtime, Review open-as/source refresh/keyboard runtime, Review shell/SRS editor command runtime, SettingsPanel navigation view-model, Settings AI view-model, Settings save/default/maintenance state projection, Settings maintenance commands, Settings props load/form projection, Settings form commands, Settings load/save commands, and Settings AI dialog/user-skill commands are already extracted, so the next safe cut should target `ReviewView.vue` Arena conflict/advisory command glue, review filter/scope command runtime, current-content editor bridge, `AiWorkbenchPane.vue` command helpers, or Browser infinite datasource/loadData controller.
