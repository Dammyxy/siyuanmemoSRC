...
- Debt deferred: `.env.example` 继续保持 AI/private/kernel transaction 默认关闭；private HTTP 暂只做 status/command，不扩展 `/review/*`、`/ai/*`、`/cards/query`；provider SSE/WS streaming 仍未声明完成；Xiuyuan / Progressive / Topic-derived broader backend-worker command ownership仍是 staged write scope，当前真实 owner 是 writer relay。
