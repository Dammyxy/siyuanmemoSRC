...
Last update: 2026-04-30 (Round 214)
...
- Debt deferred: D-10 / D-52 的大型 active 文件债继续存在；`SettingsPanel.vue` 仍保留 refs、props load/save emit、AI tool group expand/toggle 与 dialog/cleanup bridge wiring，`AIWorkbenchService.ts`、`SRSBrowser.vue`、`ReviewView.vue`、`AiWorkbenchPane.vue` 也仍待按 active behavior boundary 继续拆；Native Riff hard-delete / multi-device concurrency、AI descriptor i18n / happy-dom 噪音 / Arena scenario registration 等产品债未混入本批。
- Why deferred: 本批只处理 Settings form command seam；继续拆 Settings load/save bridge 或跨 Browser / Review / AI runtime 会扩大回归面，需要独立验收。
- Next safe step: 回到 Browser neural command/loading orchestration、`ReviewView.vue` more-menu command grouping，或继续收敛 SettingsPanel 的 load/save bridge（需单独验收）。
...
- Why deferred: 本批只处理 Settings maintenance command seam 和同一路由的死事件面；继续拆 Browser / Review / AI runtime 或 Settings load/save bridge 会扩大回归面，需要独立验收。
- Next safe step: 回到 Browser neural command/loading orchestration、`ReviewView.vue` more-menu command grouping，或继续 `SettingsPanel.vue` load/save bridge extraction。
...
| P1 | Large active files still mix state, command, projection, and persistence concerns; Browser chrome preference/open-state hydration, AI workbench session runtime mechanics, AI workbench pane CDF/self-test/message render projection, Review AI sidecar/companion command orchestration, SettingsPanel tab/subtab navigation view-model, Settings AI prompt/user-skill view-model, Settings save/default/maintenance state projection, Settings maintenance commands, Settings props load/form projection, Settings form commands, and Settings AI dialog/user-skill commands have been split out, but larger command/loading/neural/tool-chain/orchestration seams remain | `AIWorkbenchService.ts`, `SRSBrowser.vue`, `ReviewView.vue`, `SettingsPanel.vue`, `AiWorkbenchPane.vue`, `src/ui/browser/browserChromePreferences.ts`, `src/ui/browser/browserSurfaceState.ts`, `src/application/services/AIWorkbenchSessionRuntime.ts`, `src/ui/ai/aiWorkbenchPaneProjection.ts`, `src/ui/review/v2/reviewAICommands.ts`, `src/ui/settings/settingsPanelViewModel.ts`, `src/ui/settings/settingsAIViewModel.ts`, `src/ui/settings/settingsSavePayload.ts`, `src/ui/settings/settingsStateDefaults.ts`, `src/ui/settings/settingsMaintenanceViewModel.ts`, `src/ui/settings/settingsMaintenanceCommands.ts`, `src/ui/settings/settingsLoadState.ts`, `src/ui/settings/settingsFormViewModel.ts`, `src/ui/settings/settingsFormCommands.ts`, `src/ui/settings/settingsAIDialogs.ts` | Continue splitting only along active behavior boundaries; next safe cut is Browser neural command/loading orchestration, `ReviewView.vue` more-menu command grouping, or SettingsPanel load/save bridge extraction, not naming-only moves |
...
1. Continue splitting the largest active files by state / command / projection / persistence boundaries; Browser chrome preference/open-state hydration, AI workbench session runtime, AI workbench pane projection, Review AI command helper, SettingsPanel navigation view-model, Settings AI view-model, Settings save/default/maintenance state projection, Settings maintenance commands, Settings props load/form projection, Settings form commands, and Settings AI dialog/user-skill commands are already extracted, so the next safe cut should target Browser neural commands / loading orchestration, `ReviewView.vue` more-menu command grouping, or SettingsPanel load/save bridge extraction.
