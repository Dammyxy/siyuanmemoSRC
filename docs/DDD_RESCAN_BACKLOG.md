...
Last update: 2026-04-30 (Round 213)
...
- Debt deferred: D-10 / D-52 的大型 active 文件债继续存在；`SettingsPanel.vue` 仍有部分 form handlers，`AIWorkbenchService.ts`、`SRSBrowser.vue`、`ReviewView.vue`、`AiWorkbenchPane.vue` 也仍待按 active behavior boundary 继续拆；Native Riff hard-delete / multi-device concurrency、AI descriptor i18n / happy-dom 噪音 / Arena scenario registration 等产品债未混入本批。
- Why deferred: 本批只处理 Settings maintenance command seam 和同一路由的死事件面；继续拆 Browser / Review / AI runtime 或 Settings form handlers 会扩大回归面，需要独立验收。
- Next safe step: 回到 Browser neural command/loading orchestration、`ReviewView.vue` more-menu command grouping，或继续 `SettingsPanel.vue` 剩余 form handler projection。
...
- Debt deferred: D-10 / D-52 的大型 active 文件债继续存在；`SettingsPanel.vue` 当时仍有 repair/optimization command state（后续 Round 213 已收口）与部分 form handlers 可继续拆，`AIWorkbenchService.ts`、`SRSBrowser.vue`、`ReviewView.vue`、`AiWorkbenchPane.vue` 也仍待按 active behavior boundary 继续拆；Native Riff hard-delete / multi-device concurrency、AI descriptor i18n / happy-dom 噪音 / Arena scenario registration 等产品债未混入本批。
...
| P1 | Large active files still mix state, command, projection, and persistence concerns; Browser chrome preference/open-state hydration, AI workbench session runtime mechanics, AI workbench pane CDF/self-test/message render projection, Review AI sidecar/companion command orchestration, SettingsPanel tab/subtab navigation view-model, Settings AI prompt/user-skill view-model, Settings save/default/maintenance state projection, Settings maintenance commands, Settings props load/form projection, and Settings AI dialog/user-skill commands have been split out, but larger command/loading/neural/tool-chain/orchestration seams remain | `AIWorkbenchService.ts`, `SRSBrowser.vue`, `ReviewView.vue`, `SettingsPanel.vue`, `AiWorkbenchPane.vue`, `src/ui/browser/browserChromePreferences.ts`, `src/ui/browser/browserSurfaceState.ts`, `src/application/services/AIWorkbenchSessionRuntime.ts`, `src/ui/ai/aiWorkbenchPaneProjection.ts`, `src/ui/review/v2/reviewAICommands.ts`, `src/ui/settings/settingsPanelViewModel.ts`, `src/ui/settings/settingsAIViewModel.ts`, `src/ui/settings/settingsSavePayload.ts`, `src/ui/settings/settingsStateDefaults.ts`, `src/ui/settings/settingsMaintenanceViewModel.ts`, `src/ui/settings/settingsMaintenanceCommands.ts`, `src/ui/settings/settingsLoadState.ts`, `src/ui/settings/settingsFormViewModel.ts`, `src/ui/settings/settingsAIDialogs.ts` | Continue splitting only along active behavior boundaries; next safe cut is Browser neural command/loading orchestration, `ReviewView.vue` more-menu command grouping, or remaining `SettingsPanel.vue` form handler projection, not naming-only moves |
...
1. Continue splitting the largest active files by state / command / projection / persistence boundaries; Browser chrome preference/open-state hydration, AI workbench session runtime, AI workbench pane projection, Review AI command helper, SettingsPanel navigation view-model, Settings AI view-model, Settings save/default/maintenance state projection, Settings maintenance commands, Settings props load/form projection, and Settings AI dialog/user-skill commands are already extracted, so the next safe cut should target Browser neural commands / loading orchestration, `ReviewView.vue` more-menu command grouping, or remaining `SettingsPanel.vue` form handler projection.
