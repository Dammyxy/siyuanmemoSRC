...
The application/backend-client lifecycle Module for long SiYuanMemo maintenance jobs coordinated with the kernel companion. Its registry owns submit/status/cancel/defer/shutdown vocabulary for work such as startup storage maintenance, Review truth flush/backfill, truth-promotion tracking, progressive excerpt completion repair, and kernel transaction action polling. Current P0 implementation routes those jobs through this registry; kernel companion code remains coordination/relay only, while scheduler, card DB, msgpack truth, Riff/card writes, and SQLite writes stay with their current owners. Xiuyuan startup sync is retired from plugin startup and must not return as passive background work.
...
The read-only application status Module over Kernel Companion Background Work registry records. It owns normalized list/get status, kind filtering, stable newest-first ordering, terminal timestamp naming, lifecycle dedupe digest display, coalesced/skipped evidence, and content-safe diagnostic redaction for startup storage maintenance, Review truth jobs, truth-promotion tracking, progressive excerpt completion repair, and kernel transaction action polling. It may consume cloned registry records, but it must not submit, cancel, defer, retry, shutdown, or expose card/block content, SQL payloads, host-effect request bodies, or raw lifecycle keys.
...
Retired plugin-start lifecycle for old passive Xiuyuan sync. Current startup must not schedule full/incremental Xiuyuan sync, native Riff compatibility scans, or Riff/card writes. Explicit Native Riff Import/Adoption flows own those user-initiated migrations instead; any future reintroduction needs a new OpenSpec change and registry lifecycle proof.
_Avoid_: registry-owned Xiuyuan planning, physical interruption claims for already-issued writes, manual sync behavior changes, passive startup sync
